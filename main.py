...
from config.metaworld import (
    args_ml1_reach_varibad,
    args_ml1_reach_hyperx,
    args_ml1_push_varibad,
    args_ml1_push_hyperx,
    args_ml1_pickplace_varibad,
    args_ml1_pickplace_hyperx,
    args_ml10_varibad,
)
...
from config.mujoco import (
    ...
    args_walker_varibad,
    args_hop_walk,
    args_hop_walk_nonpara,
    args_hop,
    args_sparse_ant_goal_varibad,
    args_sparse_cheetah_dir_varibad,
)
...
def main():
    ...
    if args.deterministic_execution:
        ...
        if args.num_processes > 1:
            raise RuntimeError(
                "If you want fully deterministic code, use num_processes 1."
    ...
    # clean up arguments
    if (
        hasattr(args, "disable_decoder")
        and args.disable_decoder
        and not args.rlloss_through_encoder
    ):
    ...
    if args.rlloss_through_encoder and args.norm_latent_for_policy:
        raise RuntimeError(
            "Cannot backprop RL-loss through encoder and normalise latents. Disable one."
        )

    if args.pass_latent_to_policy and args.pass_task_to_policy:
        print(
            "Warning: You are passing both the task and latent to the policy. This may not be intended."
        )

    if "metaworld" in args.env_name and "norm_rew_clip_param" not in args:
        raise ValueError(
            "Looks like you're using MetaWorld. "
            "Please specify --norm_rew_clip_param in your args!"
        )

    if "PlanningGame" in args.env_name:
        assert args.max_rollouts_per_task == 1, (
            "If you want to use the planning game with > 1 rollout per task,"
