...
class Obs:
    def __init__(self, check_up, check_down):
        ...
        self.END_CHECK = (
            0  # Special case for the end, if there is not be an indicator there
        )
        self.CHECK2OBS = {
    ...
    def get_observation_space(
        ...
        wave_encoding_len,
        timeout,
        num_indicators_components=None,
    ):
        if intermediate_indicators:
            ...
            indicator_low = np.broadcast_to(
                indicator_component_low, (num_indicators_components)
            )
            indicator_hi = np.broadcast_to(
                indicator_component_hi, (num_indicators_components)
            )
...
class TMaze(gym.Env):
    ...
    def __init__(self, config):
        ...
        assert given_args == required_args, "Errors on: {}".format(
        ...
        assert not (self.force_right and self.allow_left), (
            "Cannot force right action and allow left action"
        )
        assert not (self.force_right and self.intermediate_checks), (
            "Cannot force right action and do intermediate checks"
        )
        assert not (self.intermediate_checks and not self.intermediate_indicators), (
            "Intermediate indicators required to do intermediate checks"
        )
        ...
        if self.maze_length_upper_bound is not None:
            ...
            assert self.maze_length_upper_bound >= self.maze_len, (
                self.maze_length_upper_bound,
                self.maze_len,
            )
        self.indicator_pos = config["indicator_pos"]
        assert self.indicator_pos >= 0 and self.indicator_pos <= self.maze_len - 1, (
            self.indicator_pos
        )
        ...
        indicator_at_endpoint = (self.indicator_pos == 0) or (
            self.indicator_pos == (self.maze_len - 1)
        )
        if self.flipped_indicator_pos is not None:
            assert self.correlated_indicator_pos is None, (
                "Cannot have correlated indicator and flipped indicator currently"
            )
            flipped_indicator_at_endpoint = (self.flipped_indicator_pos == 0) or (
                self.flipped_indicator_pos == (self.maze_len - 1)
            )
            ...
            assert not (self.flipped_indicator_pos == self.indicator_pos), (
                "Cannot have both long term indicators in the same location"
            )
        if self.correlated_indicator_pos is not None:
            correlated_indicator_at_endpoint = (self.correlated_indicator_pos == 0) or (
                self.correlated_indicator_pos == (self.maze_len - 1)
            )
            assert not (indicator_at_endpoint or correlated_indicator_at_endpoint), (
                "This is probably won't test the order dependence I want, so not implemented."
            )
            assert not (self.correlated_indicator_pos == self.indicator_pos), (
                "Cannot have both long term indicators in the same location"
            )
        if self.agreement_indicator_pos is not None:
            assert self.correlated_indicator_pos is None, (
                "Cannot have multiple secondary indicators"
            )
            assert self.flipped_indicator_pos is None, (
                "Cannot have multiple secondary indicators"
            )
            agreement_indicator_at_endpoint = (self.agreement_indicator_pos == 0) or (
                self.agreement_indicator_pos == (self.maze_len - 1)
            )
            assert not (indicator_at_endpoint or agreement_indicator_at_endpoint), (
                "This is probably won't test the order dependence I want, so not implemented."
            )
            assert not (self.agreement_indicator_pos == self.indicator_pos), (
                "Cannot have both long term indicators in the same location"
            )
        ...
        self.reset_task(
            None, return_obs=False
        )  # obs not yet initialized until next line
        self.reset()
    ...
    def get_latent_p(self):
        ...
        p = (
            ...
            else self.latent_dist[1]
        )  # select p based on task
        return p

    def get_random_indicator(self):
        return self.np_random.choice(
            [Indicator_Dir.UP, Indicator_Dir.DOWN], p=[0.5, 0.5]
        )

    def get_task(self):
        return (
            np.array([self.indicator])
            if self.secondary_indicator is None
            else np.array([self.indicator, self.secondary_indicator])
        )
    ...
    def task_to_id(self, tasks):
        ...
        if self.secondary_indicator is None:
            ids = torch.where(
                tasks == Indicator_Dir.UP,
                torch.ones_like(tasks),
                torch.zeros_like(tasks),
            )
        else:
            ids = torch.where(
                tasks[:, 0] == Indicator_Dir.UP,
                torch.ones_like(tasks[:, 0]),
                torch.zeros_like(tasks[:, 0]),
            )
            ids = ids + 2 * torch.where(
                tasks[:, 1] == Indicator_Dir.UP,
                torch.ones_like(tasks[:, 1]),
                torch.zeros_like(tasks[:, 1]),
            )
        return ids
    ...
    def reset(self, seed=None, options=None):
        ...
        if self.intermediate_indicators:
            random = (
                self.np_random
                if self.reset_intermediate_indicators
                else np.random.RandomState(0)
            )
            if self.latent_dist is None:
                self.checks = random.choice(
                    [Check_Dir.UP, Check_Dir.DOWN],
                    size=(self.maze_len, self.num_indicators_components),
                    p=[0.5, 0.5],
                )
            else:
                self.checks = random.choice(
                    [Check_Dir.UP, Check_Dir.DOWN],
                    size=(self.maze_len, self.num_indicators_components),
                    p=[self.get_latent_p(), 1 - self.get_latent_p()],
                )
    ...
    def reset_task(self, task, return_obs=True):
        if self.maze_length_upper_bound is not None:
            self.maze_len = self.np_random.randint(
                low=self.maze_length_lower_bound, high=self.maze_length_upper_bound + 1
            )
        ...
        if self.flipped_indicator_pos is not None:
            ...
            self.secondary_indicator = (
                Indicator_Dir.UP
                if self.indicator == Indicator_Dir.DOWN
                else Indicator_Dir.DOWN
            )
        elif self.correlated_indicator_pos is not None:
            self.secondary_indicator_pos = self.correlated_indicator_pos
            self.secondary_indicator = (
                self.get_random_indicator() if task is None else task[1]
            )  # This is correlated with goal in that DD,UU,DU -> D; UD -> U
    ...
    def add_check(self, obs):
        ...
        if self.get_cur_check() is None:
            ...
            check_obs = [
                self.OBS.END_CHECK for _ in range(self.num_indicators_components)
            ]
        else:
    ...
    def add_positional_encoding(self, obs):
        ...
        if not self.pos_enc:
            return obs
        return obs + (
            self.pos_wave_encoding(self.timestep, self.wave_encoding_len, self.timeout)
            if (self.wave_encoding_len is not None)
            else [self.timestep]
        )
    ...
    def get_obs_without_check(self):
        ...
        # Middle
        if (self.indicator_pos != self.cur_pos) and (
            self.secondary_indicator_pos != self.cur_pos
        ):  # Indicator not here
    ...
    def get_obs(self):
        ...
        return np.array(
            self.add_positional_encoding(self.add_check(self.get_obs_without_check()))
        )
    ...
    def reset_cur_check(self):
        if self.checks[self.cur_pos] is not None:
            if self.latent_dist is None:
                self.checks[self.cur_pos] = self.np_random.choice(
                    [Check_Dir.UP, Check_Dir.DOWN],
                    size=(self.num_indicators_components),
                    p=[0.5, 0.5],
                )
            else:
                self.checks[self.cur_pos] = self.np_random.choice(
    ...
    def step(self, action):
        ...
        if self.timeout is not None and self.timestep > self.timeout - 1:
            ...
            return self.get_obs(), self.timeout_reward, False, True, {}
        ...
        if a == Actions.LEFT:
            ...
            if self.cur_pos == self.maze_len - 1:  # At end
                ...
                if (self.correlated_indicator_pos is not None) or (
                    self.agreement_indicator_pos is not None
                ):
                    indicator_pattern = (self.indicator, self.secondary_indicator)
                    if (
                        self.correlated_indicator_pos is not None
                    ):  # DD,UU,DU -> D; UD -> U
                    ...
                    assert indicator_pattern in (up_patterns + down_patterns), (
                        indicator_pattern
                    )
                    success = (
                        (a == Actions.UP) and (indicator_pattern in up_patterns)
                    ) or ((a == Actions.DOWN) and (indicator_pattern in down_patterns))
                else:
                    success = (
                        a == self.indicator
                    )  # Only one indicator and agent is correct
        ...
        if (
            self.intermediate_checks
            and (self.cur_pos < self.maze_len - 1)
            and not moved_left
        ):
        ...
        if (
            self.intermediate_indicators
            and self.reset_intermediate_indicators
            and self.per_step_reset
        ):
            self.reset_cur_check()
        return self.get_obs(), reward, terminated, truncated, {"task": self.get_task()}
