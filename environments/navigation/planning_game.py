...
class MemoryPlanningGame(gym.Env):
    ...
    def get_obs(self):
        ...
        obs = np.concatenate(
            (self._one_hot(self.position), self._one_hot(self.goal)), axis=-1
        )
        ...
        if self.show_xy:
            obs = np.concatenate(
                (obs, np.array(self.position, dtype=np.float32)), axis=-1
            )
    ...
    def reset(self, seed=None, options=None):
        ...
        self._labels = {n: random_labels[i] for i, n in enumerate(self._graph.nodes())}
        self._respawn()
        self._set_new_goal()
        return self.get_obs(), {}
