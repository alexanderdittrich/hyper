...
class PointEnv(Env):
    ...
    def sample_task(self):
        goal = np.array([np.random.uniform(-1.0, 1.0), np.random.uniform(-1.0, 1.0)])
        return goal
...
class SemicircleEnv(Env):
    ...
    def __init__(self, goal_radius=0.04, radius=0.2, max_episode_steps=10):
        self.radius = radius
        self.observation_space = spaces.Box(low=-np.inf, high=np.inf, shape=(3,))
        self.action_space = spaces.Box(
            low=np.array([-0.01, -2 * np.pi / 100]),
            high=np.array([0.01, 2 * np.pi / 100]),
        )
        self._max_episode_steps = max_episode_steps
        self.goal_radius = goal_radius
        self.task_dim = 3
    ...
    def step(self, action):
        for _ in range(10):
            ...
            x = min(x, self.radius)
            x = max(x, -self.radius)
            y = min(y, abs((self.radius**2 - x**2)) ** 0.5)
            y = max(y, -(abs((self.radius**2 - x**2)) ** 0.5))
            self._state = np.array([x, y, angle % (2 * np.pi)])
            # make reward sparse and reset to origin
            distance_to_goal = (
                (self._goal[0] - x) ** 2 + (self._goal[1] - y) ** 2
            ) ** 0.5
        ...
        return self._get_obs(), reward, terminated, truncated, {"task": self.get_task()}
        ...
        **kwargs,
        ...
        for episode_idx in range(num_episodes):
            ...
            for step_idx in range(1, env._max_episode_steps + 1):
                ...
                if rew > 0:
                    pos_rewarded[episode_idx].append(
                        pos[episode_idx][-2]
        ...
        for i in range(num_episodes):
            ...
            plt.plot(
                [0, 0.02 * np.cos(start_angle)],
                [0, 0.02 * np.sin(start_angle)],
                "r",
                linestyle="-",
            )
