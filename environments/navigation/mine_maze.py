...
class MineMaze(gym.Env):
    ...
    def __init__(self, config, hide_signal=False):
        ...
        self.observation_space = Box(
            low=-1.0, high=1.0, shape=(DOWNSAMPLE_SZ * DOWNSAMPLE_SZ * 3,)
        )

        self.task_dim = 1

        len_phase0 = 2 if self.multi_step_indicator else self.num_single_step_repeats
        self.phase_2_valid_xy = [
            set([(x, 0) for x in range(len_phase0)]),  # phase 0
            {
                (-1, 1),
                (-1, 2),
                (-1, 3),
    ...
    def reset_task(self, task=None, return_obs=True):
        if task is None:
            ...
            self.indicator = self.np_random.choice(
                [Indicator_Dir.UP, Indicator_Dir.DOWN], p=up_down_probs
            )
        else:
        ...
        if self.multi_step_indicator:
            self.x_2_icolor = (
                ...
                if self.indicator_is_up()
                    ...
                    self.np_random.choice([0, 1, 2])
                ]
            )
        else:
            color = "g" if self.indicator_is_up() else "r"
            self.x_2_icolor = tuple(
                [color for _ in range(self.num_single_step_repeats)]
            )
    ...
    def read_in_imgs(self, hi_res):
        img_dir = (
            "mine_maze_data/1000_resolution"
            if hi_res
            else "mine_maze_data/" + LOW_RES_STR
        )
        ...
        try:
            ...
            room_type_2_img_path = [
                glob.glob(img_dir + "/room0/" + "*.png"),
                glob.glob(img_dir + "/room1/" + "*.png"),
            ]
            ...
            print(
                "img dir",
                img_dir,
                "not found. Please run script with --downsample first",
            )
            exit()
    ...
    def get_delta_x_delta_y(self, action):
        ...
        if self.phase == 0:
            ...
            if self.xy() == (0, 0):
                ...
                raise ValueError(
                    "Agent pos: " + str(self.xy()) + " was not dealt with properly"
                )
        else:
            assert self.phase == 2, self.phase
            delta_y = 0  # Cannot move forard here
    ...
    def step(self, action):
        if self.timeout is not None and self.step_num >= self.timeout:
            return self.get_obs(), 0.0, False, True, {}
        ...
        if (
            moved
            and not self.phase == 2
            and not (self.phase == 1 and self.xy() == (0, 1))
        ):
...
def env_loop(env, config, args):
    ...
    for i in range(args.episodes):
        ...
        if args.solve:
            ...
            expected_tot_r = 0
            expected_tot_r += (num_steps - config["num_rooms"] - 1) * config[
                "reward_per_progress"
            ]
            expected_tot_r += config["num_rooms"] * config["check_success_r"]
            if args.subopt:
                if config["multi_step_indicator"]:
                    expected_tot_r += (3 / 4) * config["success_r"] + (1 / 4) * config[
                        "fail_r"
                    ]
                else:
...
def main():
    ...
    if args.downsample:
        ...
        files_to_downsample = glob.glob(
            os.path.join(LOW_RES_DIR, "**/*.png"), recursive=True
        )
