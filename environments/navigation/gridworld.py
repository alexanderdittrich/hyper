...
class GridNavi(gym.Env):
    def __init__(
        ...
        ring=False,
        hall_with_door_every=None,
        new_state_r=None,
        stuck=False,
        distance_reward=False,
        show_goal_at_start=False,
    ):
        if starting_state is None:
            assert not ring and hall_with_door_every is None, (
                "Random start location not supported for ring or hall"
            )
        ...
        self.observation_space = (
            spaces.Box(low=0, high=self.num_cells - 1, shape=(2,))
            if hall_with_door_every is None
            else spaces.Box(
        ...
        self.num_tasks = (
            num_cells**2
        )  # Note: this should really be len(self.possible_goals), but that complicates task_to_id()
    ...
    def step(self, action):
        ...
        if self._env_state[0] == self._goal[0] and self._env_state[1] == self._goal[1]:
            ...
            reward = (
                -abs(self._env_state[0] - self._goal[0])
                - abs(self._env_state[1] - self._goal[1])
            ) / self.num_cells
        else:
            reward = -0.1
        ...
        if self.show_goal_at_start:
            assert self.starting_state is not None, (
                "Start state must be specified to show goal there"
            )
            if (
                self._env_state[0] == self.starting_state[0]
                and self._env_state[1] == self.starting_state[1]
            ):
    ...
    def render(self):
        ...
        plt.plot(
            goal[0] + 0.5,
            num_cells_y - goal[1] - 1 + 0.5,
            "gx",
            markersize=15,
            zorder=5,
        )  # Changed to green for visibility
