...
from environments.garage import Environment, EnvSpec, EnvStep, StepType
from environments.garage.envs.dm_control.dm_control_viewer import DmControlViewer
...
class DMControlEnv(Environment):
    ...
    def __init__(self, env, name=None):
        ...
        if (len(action_spec.shape) == 1) and (
            -np.inf in action_spec.minimum or np.inf in action_spec.maximum
        ):
            self._action_space = akro.Discrete(np.prod(action_spec.shape))
        else:
            self._action_space = akro.Box(
