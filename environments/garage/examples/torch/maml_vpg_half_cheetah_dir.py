...
def maml_vpg_half_cheetah_dir(ctxt, seed, epochs, episodes_per_task, meta_batch_size):
    ...
    set_seed(seed)
    env = normalize(
        GymEnv(HalfCheetahDirEnv(), max_episode_length=100), expected_action_scale=10.0
    )
    ...
    task_sampler = SetTaskSampler(
        ...
        wrapper=lambda env, _: normalize(
            ...
            expected_action_scale=10.0,
        ),
    )

    meta_evaluator = MetaEvaluator(
        test_task_sampler=task_sampler, n_test_tasks=1, n_test_episodes=10
    )
    ...
    trainer.train(n_epochs=epochs, batch_size=episodes_per_task * max_episode_length)
