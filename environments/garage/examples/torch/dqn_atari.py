...
from environments.garage.torch.algos import DQN
from environments.garage.torch.policies import DiscreteQFArgmaxPolicy
from environments.garage.torch.q_functions import DiscreteCNNQFunction
...
def main(
    ...
    if n_steps is not None:
        hyperparams["n_epochs"] = math.ceil(
            int(n_steps)
            / (hyperparams["steps_per_epoch"] * hyperparams["sampler_batch_size"])
        )
...
def dqn_atari(
    ...
    **kwargs,
    ...
    replay_buffer = PathBuffer(capacity_in_transitions=hyperparams["buffer_size"])
    ...
    qf = DiscreteCNNQFunction(
        ...
        hidden_w_init=(lambda x: torch.nn.init.orthogonal_(x, gain=np.sqrt(2))),
