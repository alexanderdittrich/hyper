...
from environments.garage.experiment import deterministic, MetaWorldTaskSampler
...
from environments.garage.torch.algos import MTSAC
from environments.garage.torch.policies import TanhGaussianMLPPolicy
...
def mtsac_metaworld_mt1_pick_place(ctxt=None, *, seed, timesteps, _gpu):
    ...
    train_task_sampler = MetaWorldTaskSampler(
        mt1, "train", lambda env, _: normalize(env)
    )
    test_task_sampler = MetaWorldTaskSampler(
        mt1_test, "train", lambda env, _: normalize(env)
    )
    n_tasks = 50
    ...
    qf1 = ContinuousMLPQFunction(
        env_spec=env.spec, hidden_sizes=[400, 400, 400], hidden_nonlinearity=F.relu
    )
