...
from environments.garage.np import (
...
class EpisodeBatch(TimeStepBatch):
    ...
    def __init__(
        ...
        if len(lengths.shape) != 1:
            raise ValueError(
                f"lengths has shape {lengths.shape} but must be a ternsor of shape (N,)"
            )

        if not (lengths.dtype.kind == "u" or lengths.dtype.kind == "i"):
            raise ValueError(
                f"lengths has dtype {lengths.dtype}, but must have an integer dtype"
            )
...
def check_timestep_batch(batch, array_type, ignored_fields=()):
    ...
    for field, value in fields.items():
        ...
        if (
            ...
            and isinstance(
                value, np.ndarray
            )  # Only numpy arrays support custom dtypes.
            and value.dtype != StepType
        ):
            raise ValueError(
                f"step_types has dtype {value.dtype} but must have dtype StepType"
            )
