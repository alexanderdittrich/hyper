...
from environments.garage import log_performance, make_optimizer
from environments.garage.np import explained_variance_1d, pad_batch_array
...
class NPO(RLAlgorithm):
    ...
    def _build_inputs(self):
        ...
        for k in self.policy.state_info_keys:
            ...
            augmented_obs_var = tf.concat([augmented_obs_var, extra_state_var], -1)

        self._policy_network = self.policy.build(augmented_obs_var, name="policy")
        self._old_policy_network = self._old_policy.build(
            augmented_obs_var, name="policy"
        )
    ...
    def _build_policy_loss(self, i):
        ...
        if self._maximum_entropy:
            with tf.name_scope("augmented_rewards"):
                rewards = i.reward_var + (self._policy_ent_coeff * policy_entropy)
        ...
        with tf.name_scope("policy_loss"):
            ...
            returns = discounted_returns(
                self._discount, self.max_episode_length, rewards
            )
    ...
    def _build_entropy_term(self, i):
        ...
        policy_entropy = tf.reshape(policy_entropy, [-1, self.max_episode_length])
    ...
    def _policy_opt_input_values(self, episodes, baselines):
        ...
        padded_actions = pad_batch_array(
