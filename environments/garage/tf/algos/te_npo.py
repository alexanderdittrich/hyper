...
from environments.garage import InOutSpec, log_performance
from environments.garage.experiment import deterministic
from environments.garage.np import (discount_cumsum, explained_variance_1d, pad_batch_array,
                       rrse, sliding_window)
from environments.garage.np.algos import RLAlgorithm
from environments.garage.tf import (center_advs, compile_function, compute_advantages,
                       concat_tensor_list, discounted_returns, flatten_inputs,
                       graph_inputs, pad_tensor_dict, positive_advs,
                       stack_tensor_dict_list)
from environments.garage.tf.embeddings import StochasticEncoder
from environments.garage.tf.optimizers import LBFGSOptimizer
from environments.garage.tf.policies import TaskEmbeddingPolicy
...
class TENPO(RLAlgorithm):
    ...
    def _init_opt(self):
        ...
        """
        # Input variables
        (pol_loss_inputs, pol_opt_inputs, infer_loss_inputs, infer_opt_inputs) = (
            self._build_inputs()
        )
    ...
    def _train_once(self, itr, episodes):
        ...
        for length in episodes.lengths:
            ...
            baseline = self._baseline.predict(
                dict(
                    observations=episodes.observations[start:stop],
            ...
            start = stop
        baselines = pad_batch_array(
            np.concatenate(baselines), episodes.lengths, self.max_episode_length
        )
        ...
        self._optimize_policy(itr, episodes, baselines, embed_eps, embed_ep_infos)

        return average_return

    def _optimize_policy(self, itr, episodes, baselines, embed_eps, embed_ep_infos):
        """Optimize policy.
        ...
        fit_paths = self._evaluate(
            policy_opt_input_values, episodes, baselines, embed_ep_infos
        )
        ...
        self._old_policy.encoder.model.parameters = self.policy.encoder.model.parameters
    ...
    def _build_inputs(self):
        ...
        with tf.name_scope("inputs"):
            obs_var = observation_space.to_tf_placeholder(name="obs", batch_dims=2)
            task_var = tf.compat.v1.placeholder(
                tf.float32, shape=[None, None, task_space.flat_dim], name="task"
            )
            ...
            action_var = action_space.to_tf_placeholder(name="action", batch_dims=2)
            reward_var = tf.compat.v1.placeholder(
                tf.float32, shape=[None, None], name="reward"
            )
            baseline_var = tf.compat.v1.placeholder(
                tf.float32, shape=[None, None], name="baseline"
            )
            ...
            embed_state_info_vars = {
                k: tf.compat.v1.placeholder(
                    tf.float32, shape=[None] * 2 + list(shape), name="embed_%s" % k
                )
                for k, shape in self.policy.encoder.state_info_specs
            }
            embed_state_info_vars_list = [
                embed_state_info_vars[k] for k in self.policy.encoder.state_info_keys
            ]

            # Inference distribution state (for RNNs)
            infer_state_info_vars = {
                k: tf.compat.v1.placeholder(
                    tf.float32, shape=[None] * 2 + list(shape), name="infer_%s" % k
                )
                for k, shape in self._inference.state_info_specs
            }
            infer_state_info_vars_list = [
                infer_state_info_vars[k] for k in self._inference.state_info_keys
            ]

        extra_obs_var = [tf.cast(v, tf.float32) for v in policy_state_info_vars_list]
        # Pylint false alarm
        # pylint: disable=unexpected-keyword-arg, no-value-for-parameter
        augmented_obs_var = tf.concat([obs_var] + extra_obs_var, axis=-1)
        extra_traj_var = [tf.cast(v, tf.float32) for v in infer_state_info_vars_list]
        ...
        inference_loss_inputs = graph_inputs(
            "InferenceLossInputs", latent_var=latent_var, valid_var=valid_var
        )
    ...
    def _build_policy_loss(self, i):
        ...
        self._old_policy_network, self._old_encoder_network = self._old_policy.build(
            i.augmented_obs_var, i.task_var, name="loss_old_policy"
        )
        self._infer_network = self._inference.build(
            i.augmented_traj_var, name="loss_infer"
        )
        ...
        # Entropy terms
        encoder_entropy, inference_ce, policy_entropy = self._build_entropy_terms(i)
        ...
        with tf.name_scope("policy_loss"):
            ...
            with tf.name_scope("surr_loss"):
                ...
                lr = tf.exp(tf.minimum(ll - old_ll, np.log(1 + self._lr_clip_range)))

                surrogate = lr * adv

                surrogate = tf.debugging.check_numerics(surrogate, message="surrogate")

            # Finalize objective function
            with tf.name_scope("loss"):
                lr_clip = tf.clip_by_value(
                    lr, 1 - self._lr_clip_range, 1 + self._lr_clip_range, name="lr_clip"
                )
            ...
            self._f_policy_kl = tf.compat.v1.get_default_session().make_callable(
                pol_mean_kl, feed_list=flatten_inputs(self._policy_opt_inputs)
            )
            ...
            returns = discounted_returns(
                self._discount, self.max_episode_length, rewards, name="returns"
            )
    ...
    def _build_entropy_terms(self, i):
        ...
        with tf.name_scope("entropy_terms"):
            ...
            with tf.name_scope("encoder_entropy"):
                encoder_dist, _, _ = self.policy.encoder.build(
                    i.task_var, name="encoder_entropy"
                ).outputs
                encoder_all_task_entropies = -encoder_dist.log_prob(i.latent_var)

                if self._use_softplus_entropy:
                    encoder_entropy = tf.nn.softplus(encoder_all_task_entropies)

                encoder_entropy = tf.reduce_mean(
                    encoder_entropy, name="encoder_entropy"
                )
        ...
        self._f_task_entropies = compile_function(
            flatten_inputs(self._policy_opt_inputs), encoder_all_task_entropies
        )
    ...
    def _build_inference_loss(self, i):
        ...
        with tf.name_scope("infer_loss"):
            ...
            traj_gammas = tf.constant(
                float(self._discount), dtype=tf.float32, shape=[self.max_episode_length]
            )
            # Pylint false alarm
            # pylint: disable=no-value-for-parameter
            traj_discounts = tf.compat.v1.cumprod(
            ...
            with tf.name_scope("loss"):
                infer_loss = -tf.reduce_mean(discount_traj_ll, name="infer_loss")
    ...
    def _policy_opt_input_values(self, episodes, baselines, embed_eps):
        ...
        """
        actions = [
            self._env_spec.action_space.flatten_n(act) for act in episodes.actions_list
        ]
        actions = pad_batch_array(
            np.concatenate(actions), episodes.lengths, self.max_episode_length
        )
        tasks = pad_batch_array(
            episodes.env_infos["task_onehot"], episodes.lengths, self.max_episode_length
        )
        latents = pad_batch_array(
            episodes.agent_infos["latent"], episodes.lengths, self.max_episode_length
        )

        agent_infos = episodes.padded_agent_infos
        policy_state_info_list = [agent_infos[k] for k in self.policy.state_info_keys]
        embed_state_info_list = [
            agent_infos["latent_" + k] for k in self.policy.encoder.state_info_keys
        ]
    ...
    def _evaluate(self, policy_opt_input_values, episodes, baselines, embed_ep_infos):
        ...
        env_returns = [
            discount_cumsum(rwd, self._discount) for rwd in episodes.padded_rewards
        ]
        env_average_discounted_return = np.mean([ret[0] for ret in env_returns])

        # Recompute returns and prepare paths for fitting the baseline
        aug_rewards = []
        aug_returns = []
        for rew, ret, val, task, latent, obs in zip(
            rewards_tensor, returns_tensor, valids, tasks, latents, observations
        ):
            ...
            fit_paths.append(
                dict(observations=obs, tasks=task, latents=latent, returns=returns)
            )
        ...
        aug_average_discounted_return = np.mean([ret[0] for ret in returns_tensor])
        d_returns = np.mean(
            aug_average_discounted_return - env_average_discounted_return
        )
        ...
        inference_rmse = (embed_ep_infos["mean"] - latents) ** 2.0
        ...
        tabular.record("{}/Encoder/Entropy".format(self.policy.name), embed_ent)
        ...
        for t in range(self.policy.task_space.flat_dim):
            ...
            tabular.record("Tasks/EpisodeLength/t={}".format(t), np.mean(lengths))
            tabular.record("Tasks/TerminationRate/t={}".format(t), pct_completed)
    ...
    def _visualize_distribution(self):
        ...
        for task in range(num_tasks):
            for i in range(self.policy.latent_space.flat_dim):
                ...
                norm = scipy.stats.norm(loc=latent_infos["mean"][task, i], scale=stds)
    ...
    def _train_policy_and_encoder_networks(self, policy_opt_input_values):
        ...
        tabular.record("{}/Encoder/KLBefore".format(self.policy.name), embed_kl_before)
    ...
    def _train_inference_network(self, inference_opt_input_values):
        ...
        infer_loss_before = self.inference_optimizer.loss(inference_opt_input_values)
        tabular.record("Inference/Loss", infer_loss_before)
        self.inference_optimizer.optimize(inference_opt_input_values)
        infer_loss_after = self.inference_optimizer.loss(inference_opt_input_values)
