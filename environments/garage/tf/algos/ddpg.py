...
class DDPG(RLAlgorithm):
    ...
    def _init_opt(self):
        ...
        with tf.name_scope(self._name):
            ...
            next_qval = self._qf.build(obs, next_action, name="policy_action_qval")
            with tf.name_scope("action_loss"):
                action_loss = -tf.reduce_mean(next_qval)
                if self._policy_weight_decay > 0.0:
                    regularizer = tf.keras.regularizers.l2(self._policy_weight_decay)
            ...
            with tf.name_scope("qval_loss"):
                ...
                if self._qf_weight_decay > 0.0:
                    regularizer = tf.keras.regularizers.l2(self._qf_weight_decay)
            ...
            f_train_qf = compile_function(
                inputs=[input_y, obs, actions], outputs=[qf_train_op, qval_loss, qval]
            )
    ...
    def _optimize_policy(self):
        ...
        clip_range = (
            -self._clip_return,
            0.0 if self._clip_pos_returns else self._clip_return,
        )
        ys = np.clip(
