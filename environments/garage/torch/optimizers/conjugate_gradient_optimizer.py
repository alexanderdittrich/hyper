...
from environments.garage.np import unflatten_tensors
...
def _build_hessian_vector_product(func, params, reg_coeff=1e-5):
    ...
    def _eval(vector):
        ...
        grad_vector_product = torch.sum(
            torch.stack([torch.sum(g * x) for g, x in zip(f_grads, unflatten_vector)])
        )
...
class ConjugateGradientOptimizer(Optimizer):
    ...
    def step(self, f_loss, f_constraint):  # pylint: disable=arguments-differ
        ...
        step_size = np.sqrt(
            2.0
            * self._max_constraint_value
            * (1.0 / (torch.dot(step_dir, f_Ax(step_dir)) + 1e-8))
        )
    ...
    def __setstate__(self, state):
        ...
        if "hvp_reg_coeff" not in state["state"]:
            warnings.warn(
                ...
                "This behavior is fixed if pickling from environments.garage>=2020.02.0."
            )
