...
class MetaWorldMultiEnvWrapper(Wrapper):  # Adapted from Garage's MultiEnvWrapper
    ...
    def __init__(
        ...
        self.n_tasks_total = (
            n_tasks_train + n_tasks_test
        )  # This is total, between training and test modes!
        self._num_tasks_here = (
            n_tasks_train if train else n_tasks_test
        )  # num tasks in this mode!
        ...
        self._active_task_index = self.sample_strategy(
            self._num_tasks_here - 1
        )  # sample new envs for each task
    ...
    def observation_space(self):
        ...
        if self._mode == "vanilla":
            ...
            return akro.Box(
                np.concatenate([env_lb, task_lb]), np.concatenate([env_ub, task_ub])
            )
    ...
    def task_space(self):
        ...
        ub = np.concatenate(
            [one_hot_ub, 1 * np.ones(3)]
        )  # length 3 goal will be added to task
    ...
    def all_task_ids(self):
        return (
            list(range(self.n_tasks_train))
            if self.train
            else list(range(self.n_tasks_train, self.n_tasks_total))
        )
    ...
    def sample_strategy(self, task_index):
        ...
        if task_index + 1 == self._num_tasks_here:
            ...
            gc.collect()
            self._task_envs = [
                env_up for env_up in self.task_sampler.sample(self._num_tasks_here)
            ]
            assert len(self.all_task_ids()) == len(self._task_envs), len(
                self.all_task_ids()
            ) == len(self._task_envs)
    ...
    def get_task(self):
        ...
        if self.train:
            ...
            task_one_hot[task_id] = (
                1  # in torch: F.one_hot(task_id, num_classes=self.num_tasks)
            )
        return np.concatenate(
            [task_one_hot, self._env.unwrapped._get_pos_goal()]
        ).astype(np.float32)
