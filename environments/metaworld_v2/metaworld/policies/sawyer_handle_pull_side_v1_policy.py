...
from environments.metaworld_v2.metaworld.policies.policy import (
    Policy,
    assert_fully_parsed,
    move,
)
...
class SawyerHandlePullSideV1Policy(Policy):
    ...
    def _desired_pos(o_d):
        ...
        if abs(pos_curr[1] - pos_button[1]) > 0.04:
            ...
            return pos_button + np.array([0.1, 0.0, -0.01])
        elif abs(pos_curr[0] - pos_button[0]) > 0.01:
            return np.array([pos_button[0] - 0.04, pos_button[1], pos_curr[2]])
        else:
            return pos_button + np.array([-0.04, 0.0, 0.1])
