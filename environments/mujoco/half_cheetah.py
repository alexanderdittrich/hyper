...
class HalfCheetahEnv(HalfCheetahEnv_):
    def _get_obs(self):
        # Gymnasium's HalfCheetah-v5 expects float64 observations
        obs = super()._get_obs()
        return obs.astype(np.float64).flatten()
    ...
    #         return data
    #     elif mode == 'human':
    #         self._get_viewer('human').render(width, height)
    ...
    def visualise_behaviour(
        ...
        **kwargs,
        ...
        torso_index = 2 if plot_z else 0
        start_pos = unwrapped_env.get_body_com("torso")[torso_index].copy()
        ...
        for episode_idx in range(num_episodes):
            ...
            for step_idx in range(1, env._max_episode_steps + 1):
                ...
                pos[episode_idx].append(
                    unwrapped_env.get_body_com("torso")[torso_index].copy()
                )
        ...
        for i in range(num_episodes):
            ...
            # (not plotting the last step because this gives weird artifacts)
            if plot_z:
                plt.plot(range(len(pos[i][:-1])), pos[i][:-1], "k")
                plt.title(
                    "task: {}".format(
                        task if args.pass_task_to_policy else env.get_task()
                    ),
                    fontsize=15,
                )
                plt.xlabel("steps (ep {})".format(i), fontsize=15)
                if i == num_episodes - 1:
                    plt.ylabel("position", fontsize=15)
                # else:
                #     plt.xticks([])
                plt.ylim(0, 2)
                plt.plot([200, 200], [0, 0], "b--", alpha=0.2)
            else:
