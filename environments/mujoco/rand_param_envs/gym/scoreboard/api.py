...
from environments.mujoco.rand_param_envs.gym import benchmark_spec, error, monitoring
from environments.mujoco.rand_param_envs.gym.scoreboard.client import resource, util
...
def upload(
    ...
    if benchmark_id:
        ...
        try:
            ...
            raise error.Error(
                "Invalid benchmark id: {}. Are you using a benchmark registered in gym/benchmarks/__init__.py?".format(
                    benchmark_id
                )
            )
        ...
        benchmark_run = resource.BenchmarkRun.create(
            benchmark_id=benchmark_id, algorithm_id=algorithm_id, tags=json.dumps(tags)
        )
...
def _upload(
    ...
    if not ignore_open_monitors:
        ...
        if len(open_monitors) > 0:
            ...
            raise error.Error(
                "Still have an open monitor on {}. You must run 'env.close()' before uploading.".format(
                    ", ".join(envs)
                )
            )
...
def upload_training_data(training_dir, api_key=None):
    ...
    if not results:
        raise error.Error(
            """Could not find any manifest files in {}.

(HINT: this usually means you did not yet close() your env.monitor and have not yet exited the process. You should call 'env.monitor.start(training_dir)' at the start of training and 'env.close()' at the end, or exit the process.)""".format(
                training_dir
            )
        )
...
def upload_training_episode_batch(
    data_sources,
    episode_lengths,
    episode_rewards,
    episode_types,
    ...
    logger.info(
        "[%s] Uploading %d episodes of training data", env_id, len(episode_lengths)
    )
...
def write_archive(videos, archive_file, env_id=None):
    if len(videos) > MAX_VIDEOS:
        raise error.Error(
            "[{}] Trying to upload {} videos, but there is a limit of {} currently. If you actually want to upload this many videos, please email gym@openai.com with your use-case.".format(
                env_id, MAX_VIDEOS, len(videos)
            )
        )

    logger.debug(
        "[%s] Preparing an archive of %d videos: %s", env_id, len(videos), videos
    )
    ...
    with tarfile.open(fileobj=archive_file, mode="w:gz") as tar:
        for video_path, metadata_path in videos:
            ...
            if not os.path.exists(video_path):
                raise error.Error(
                    "[{}] No such video file {}. (HINT: Your video recorder may have broken midway through the run. You can check this with `video_recorder.functional`.)".format(
                        env_id, video_path
                    )
                )
            elif not os.path.exists(metadata_path):
                raise error.Error(
                    "[{}] No such metadata file {}. (HINT: this should be automatically created when using a VideoRecorder instance.)".format(
                        env_id, video_path
                    )
                )
            ...
            if video_name in basenames:
                ...
                raise error.Error(
                    "[{}] Duplicated metadata file name {} in video list: {}".format(
                        env_id, metadata_name, videos
                    )
                )
                ...
                raise error.Error(
                    "[{}] Invalid metadata file name {} (must match {})".format(
                        env_id, metadata_name, metadata_name_re.pattern
                    )
                )
