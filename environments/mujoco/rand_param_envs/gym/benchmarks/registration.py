...
class Task(object):
    def __init__(
        self, env_id, trials, max_timesteps, max_seconds, reward_floor, reward_ceiling
    ):
        self.env_id = env_id
        self.trials = trials
        ...
        if max_timesteps is None and max_seconds is None:
            raise error.Error(
                "Must provide at least one of max_timesteps and max_seconds for {}".format(
                    self
                )
            )
...
class Benchmark(object):
    ...
    def task_specs(self, env_id):
        ...
        if not results:
            raise error.Unregistered(
                "No task with env_id {} registered for benchmark {}", env_id, self.id
            )
        return results
    ...
    def score_evaluation(
        ...
        return self.scorer.score_evaluation(
            self,
            env_id,
            data_sources,
            initial_reset_timestamps,
...
BenchmarkView = collections.namedtuple(
    "BenchmarkView", ["name", "benchmarks", "primary", "group"]
)
...
class Registry(object):
    ...
    def register_benchmark_view(self, name, benchmarks, primary, group):
        ...
        if group is None:
            group = "Miscellaneous"
        bw = BenchmarkView(
            name=name, benchmarks=benchmarks, primary=primary, group=group
        )
    ...
    def register_benchmark(
        ...
        self.benchmarks[id] = Benchmark(
            id=id, scorer=scorer, tasks=tasks, name=name, description=description
        )
        if add_view:
            self.register_benchmark_view(
    ...
    def benchmark_spec(self, id):
        try:
            ...
            raise error.UnregisteredBenchmark(
                "No registered benchmark with id: {}".format(id)
            )
