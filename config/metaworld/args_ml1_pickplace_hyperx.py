...
def get_args(rest_args):
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="number of MDP episodes for adaptation",
    )
    parser.add_argument(
        "--exp_label", default="hyperx", help="label (typically name of method)"
    )
    parser.add_argument(
        "--env_name", default="metaworld_ml1", help="environment to train on"
    )
    parser.add_argument(
        "--ml1_type", default="pick-place", help="push/pick-place/reach"
    )
    parser.add_argument(
        "--mw_version", type=int, default=1, help="version of meta-world"
    )
    parser.add_argument(
        "--sparse_metaworld_rewards",
        type=boolean_argument,
        default=False,
        help="Use sparse metaworld rewards based on success metric",
    )
    ...
    parser.add_argument(
        ...
        type=boolean_argument,
        default=True,
        help="condition policy on state",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--norm_state_for_policy",
        type=boolean_argument,
        default=True,
        help="normalise state input",
    )
    parser.add_argument(
        "--norm_latent_for_policy",
        type=boolean_argument,
        default=True,
        help="normalise latent input",
    )
    parser.add_argument(
        "--norm_belief_for_policy",
        type=boolean_argument,
        default=True,
        help="normalise belief input",
    )
    parser.add_argument(
        "--norm_task_for_policy",
        type=boolean_argument,
        default=True,
        help="normalise task input",
    )
    parser.add_argument(
        "--norm_rew_for_policy",
        type=boolean_argument,
    ...
    parser.add_argument(
        "--norm_actions_pre_sampling",
        type=boolean_argument,
        default=False,
        help="normalise policy output",
    )
    parser.add_argument(
        "--norm_actions_post_sampling",
        type=boolean_argument,
        default=True,
        help="normalise policy output",
    )
    parser.add_argument(
        "--norm_rew_clip_param", type=float, default=100000, help="rew clip param"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="tanh",
        help="tanh/relu/leaky-relu",
    )
    parser.add_argument(
        "--policy_initialisation", type=str, default="normc", help="normc/orthogonal"
    )
    parser.add_argument(
        "--policy_anneal_lr",
        type=boolean_argument,
        default=False,
        help="anneal LR over time",
    )

    # RL algorithm
    parser.add_argument(
        "--policy_optimiser", type=str, default="adam", help="choose: rmsprop, adam"
    )

    # PPO specific
    parser.add_argument(
        "--ppo_num_epochs", type=int, default=2, help="number of epochs per PPO update"
    )
    parser.add_argument(
        "--ppo_num_minibatch",
        type=int,
    ...
    parser.add_argument(
        ...
        type=boolean_argument,
        default=True,
        help="use huberloss instead of MSE",
    )
    parser.add_argument(
        "--ppo_use_clipped_value_loss",
        type=boolean_argument,
        default=True,
    ...
    parser.add_argument(
        "--lr_policy", type=float, default=7e-4, help="learning rate (default: 7e-4)"
    )
    parser.add_argument(
        "--num_processes",
        type=int,
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="only used for continuous actions",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--policy_gamma", type=float, default=0.97, help="discount factor for rewards"
    )
    parser.add_argument(
        "--policy_use_gae",
        type=boolean_argument,
    ...
    parser.add_argument(
        "--policy_max_grad_norm", type=float, default=0.5, help="max norm of gradients"
    )
    ...
    parser.add_argument(
        ...
        type=boolean_argument,
        default=False,
        help="use weighted sampling for trajectories from VAE buffer",
    )
    parser.add_argument(
        "--vae_reward_threshold",
        type=float,
        default=0.00001,
        help="minimum threshold for counting relevant rewards",
    )
    parser.add_argument(
        "--sample_reward_factor",
        type=float,
        default=10,
        help="constant that increases likelihood of sampling a reward-dense trajectory",
    )
    ...
    parser.add_argument(
        "--latent_dim", type=int, default=5, help="dimensionality of latent space"
    )
    ...
    parser.add_argument(
        "--normalise_rew_targets",
        type=boolean_argument,
        default=True,
        help="divide reward targets by largest rew seen",
    )
    parser.add_argument(
        "--rew_loss_coeff",
        type=float,
    ...
    parser.add_argument(
        "--reward_decoder_layers", nargs="+", type=int, default=[128, 64, 32]
    )
    parser.add_argument(
        "--multihead_for_reward",
        type=boolean_argument,
        default=False,
    ...
    parser.add_argument(
        "--decode_state", type=boolean_argument, default=True, help="use state decoder"
    )
    parser.add_argument(
        "--state_loss_coeff", type=float, default=1.0, help="weight for state loss"
    )
    parser.add_argument(
        "--state_decoder_layers", nargs="+", type=int, default=[128, 64, 32]
    )
    parser.add_argument(
        "--state_pred_type",
        type=str,
        default="deterministic",
        help="choose: deterministic, gaussian",
    )

    # - decoder: ground-truth task ("varibad oracle", after Humplik et al. 2019)
    parser.add_argument(
        "--decode_task", type=boolean_argument, default=False, help="use task decoder"
    )
    parser.add_argument(
        "--task_loss_coeff", type=float, default=1.0, help="weight for task loss"
    )
    ...
    parser.add_argument(
        "--disable_stochasticity_in_latent",
        type=boolean_argument,
        default=False,
    ...
    parser.add_argument(
        "--intrinsic_rew_type", type=str, default="random_network_distillation"
    )

    # what to compute the reward bonus for
    parser.add_argument(
        "--intrinsic_rew_for_hyperstate",
        type=boolean_argument,
        default=True,
        help="reward bonus on (s, b)",
    )
    parser.add_argument(
        "--intrinsic_rew_for_state",
        type=boolean_argument,
        default=False,
        help="reward bonus only on (s)",
    )
    parser.add_argument(
        "--intrinsic_rew_for_belief",
        type=boolean_argument,
        default=False,
        help="reward bonus only on (b)",
    )
    parser.add_argument(
        "--intrinsic_rew_include_actions",
        type=boolean_argument,
        default=False,
        help="also reward novel actions (rew bonus for (s,b,a))",
    )
    # whether or not to normalise intrinsic rewards
    parser.add_argument(
        "--intrinsic_rew_normalise_rewards", type=boolean_argument, default=True
    )
    parser.add_argument("--intrinsic_rew_clip_rewards", type=float, default=10.0)
    parser.add_argument(
        "--intrinsic_rew_normalise_rew_sum",
        type=boolean_argument,
        default=False,
        help="normalise the sum of intrinsic and extrinsic reward",
    )
    # weights for the rewards bonuses
    parser.add_argument(
        "--rnd_weight_hyperstate",
        type=float,
        default=1.0,
        help="weight of additional reward / value",
    )
    parser.add_argument(
        "--rnd_weight_state",
        type=float,
    ...
    # anneal the weight of the bonuses over the course of training
    parser.add_argument(
        "--intrinsic_rew_anneal_weight", type=boolean_argument, default=True
    )

    # VAE rew reconstruction loss as reward
    parser.add_argument(
        "--intrinsic_rew_for_vae_loss", type=boolean_argument, default=True
    )
    parser.add_argument(
        "--intrinsic_weight_vae_loss",
        type=float,
        default=1.0,
        help="weight of additional reward",
    )

    # hyperparameters for the random network
    parser.add_argument("--lr_rnd", type=float, default=1e-4, help="learning rate ")
    parser.add_argument("--rnd_batch_size", type=int, default=128)
    parser.add_argument("--rnd_update_frequency", type=int, default=50)
    parser.add_argument("--size_rnd_buffer", type=int, default=10000)
    parser.add_argument("--rnd_buffer_add_thresh", type=float, default=1.0)
    parser.add_argument("--rnd_output_dim", type=int, default=128)
    parser.add_argument("--layers_rnd_prior", nargs="+", type=int, default=[256, 256])
    parser.add_argument(
        "--layers_rnd_predictor", nargs="+", type=int, default=[256, 256]
    )
    parser.add_argument(
        "--rnd_use_orthogonal_init",
        type=boolean_argument,
        default=False,
        help="use orthogonal weight initialisation",
    )
    parser.add_argument("--rnd_norm_inputs", type=boolean_argument, default=False)
    parser.add_argument(
        "--rnd_init_weight_scale",
        type=float,
        default=10.0,
        help="by how much to scale the random network weights",
    )

    #  for the cheetah-dir env - use only x-position
    parser.add_argument(
        "--state_expl_idx",
        nargs="+",
        type=int,
