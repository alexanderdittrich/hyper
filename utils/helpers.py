...
def make_env(args, mode="train", train_task_override=None, **kwargs):
    ...
    if env_id.startswith("metaworld"):
        ...
        if env_id == "metaworld_ml1":
            ...
            if mode == "train":
                ...
                if train_task_override is not None:
                    env.reset_task = lambda: env.set_task(
                        random.choice(train_task_override)
                    )
                else:
                    env.reset_task = lambda: env.set_task(
                        random.choice(mworld.train_tasks)
                    )
            elif mode == "test":
        ...
        # --- ML10 ---
            ...
            from environments.garage.experiment.task_sampler import (
                MetaWorldTaskSampler,
            )  # Can't do this at top since it breaks MuJoCo131 needed for Walker
            ...
            from environments.mw_wrapper import (
                MetaWorldMultiEnvWrapper,
            )  # Can't do this at top since it breaks MuJoCo131 needed for Walker

            env = MetaWorldMultiEnvWrapper(
                task_sampler,
                n_tasks_train=10,
    ...
    # OTHERWISE WE ASSUME ITS A GYM ENV
        ...
        if args is not None and args.env_name == "TreasureHunt-v0":
            ...
            env = gym.make(env_id, level=args.level, **kwargs)
        else:
            env = gym.make(env_id, **kwargs)
...
def reset_env(env, args, indices=None, state=None):
    ...
    if (indices is None) or (len(indices) == args.num_processes):
        ...
        if isinstance(
            state, tuple
        ):  # Handle both VecEnv (returns obs) and Gym (returns obs, info)
            state = state[0]
    ...
    # reset only the ones given by indices
        ...
        for i in indices:
            state_i = env.reset(index=i)
            if isinstance(state_i, tuple):  # Handle both VecEnv and Gym interfaces
                state_i = state_i[0]
            state[i] = state_i

    belief = (
        torch.from_numpy(env.get_belief()).to(device)
        if args.pass_belief_to_policy
        else None
    )
    decode_task_in_rlloss = args.decode_task and args.rlloss_through_encoder
    task = (
        torch.from_numpy(env.get_task()).to(device)
        if args.pass_task_to_policy or decode_task_in_rlloss
        else None
    )
...
def get_num_tasks(args):
    ...
    try:
        ...
        try:
            ...
            num_tasks = env.unwrapped.num_tasks
        except AttributeError:
            # Silently set to None for continuous task distributions
            num_tasks = None
    return num_tasks
