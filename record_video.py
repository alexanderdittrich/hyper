...
MC_END_FRAME_REPEAT = (
    0  # repeat signal in video for MineCraft to make it clear to human
)
...
def write_video(
    ...
    out_path,
    fps=3,
    end_repeat=MC_END_FRAME_REPEAT,
    start_repeat=MC_START_FRAME_REPEAT,
):
    # From: https://github.com/jacooba/AMRL-ICLR2020
    observations = [
        o[:-1].reshape(10, 10, 3).numpy() for o in observations
    ]  # unflatten and remove extra number
...
def rollout_varibad(
    ...
    activation_steps,
    activations_by_step,
    activation_num_tasks,
    activation_num_saves,
    render=False,
    num_episodes=2,
):
    ...
    # get policy network
    policy = torch.load(
        os.path.join(path_to_results, "models", "policy.pt"), map_location="cpu"
    ).to(device)
    # get encoder
    encoder = torch.load(
        os.path.join(path_to_results, "models", "encoder.pt"), map_location="cpu"
    ).to(device)
    # get the normalisation parameters for the environment
    ret_rms = (
        None  # utl.load_obj(os.path.join(path_to_results, 'models'), "env_rew_rms")
    )
    ...
    if video:
        if "PlanningGame" in args.env_name:
            ...
            envs.envs[0] = RecordVideo(
                envs.envs[0], out_path, episode_trigger=lambda x: True
            )
    ...
    if (
        args.env_name == "metaworld_ml10" and plot_activations
    ):  # just get non-parametric task:
        tasks = tasks[:, :10]
    ...
    for step in range(max_episode_steps):
        ...
        with torch.no_grad():
            _, action, _ = utl.select_action(
                ...
                deterministic=True,
                latent_sample=latent_sample.to(device),
        ...
        if plot_activations and step in activation_steps:
            ...
            for env_i in range(len(tasks)):
                ...
                activations = policy.forward(
                    state[env_i].to(device),
                    latent[env_i].to(device),
                    None,
                    None,
                ...
                g = tuple(
                    tasks[env_i]
                )  # for some envs, may be: envs.envs[0].get_task()
            ...
            activations_by_step.append(
                goal_2_activations
            )  # also save that dict by timestep

        # Observe reward and next obs
        (state, belief, task), (rew_raw, rew_normalised), done, info = utl.env_step(
            envs, action, args
        )
        if video and "MC" in args.env_name:
            if rew_raw == 4 or rew_raw == -3:
                ep_observations = ep_observations + [
                    state[0] for _ in range(MC_START_FRAME_REPEAT)
                ]
            print(rew_raw)
        # May also need:
        ...
        latent_sample, latent_mean, latent_logvar, hidden_state = utl.update_encoding(
            encoder, state, action, rew_raw, prev_state, done, hidden_state
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--path", default="/Users/jake/Desktop/HopWalk_Behavior/HopWalk_Behavior20-hop"
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="whether to plot activations of the policy network instead of video",
    )
    parser.add_argument(
        "--reduce_dims_by_task",
        action="store_true",
        default=False,
        help="whether to do dimension reduction by task if plotting activations",
    )
    parser.add_argument(
        "--tsne",
        action="store_true",
        default=False,
        help="wether to use TSNE instead of PCA for dim reduction of activations",
    )
    parser.add_argument(
        "--activation_path", default="/users/jaceck/MetaMem/activations_varibad"
    )
    parser.add_argument(
        "--activation_num_saves",
        type=int,
        default=10,
        help="number of times to save activations over rollout (plus end)",
    )  # [0, 100, 199]
    parser.add_argument("--activation_num_tasks", type=int, default=100)
    parser.add_argument(
        "--num_episodes", type=int, default=1
    )  # Note this may break things if not equal to the number during training
    ...
    returns = rollout_varibad(
        path_to_results,
        args.out_path,
        args.plot_activations,
        args.activation_path,
        activation_steps,
        activations_by_step,
        args.activation_num_tasks,
        args.activation_num_saves,
    ...
    if args.plot_activations:
        ...
        if args.reduce_dims_by_task:
            for task_id in range(
                num_task_ids
            ):  # note: relies on same number of tasks_id at ever step
                data_in_task = task_id_2_all_data[task_id]
                dim_reduction = (
                    TSNE(n_components=2) if args.tsne else PCA(n_components=2)
                )
