...
from config.gridworld import (
    args_grid_oracle,
    args_grid_belief_oracle,
    args_grid_rl2,
    args_grid_varibad,
    args_TLN_varibad,
)
from config.mujoco import (
    args_cheetah_dir_oracle,
    args_cheetah_dir_rl2,
    args_cheetah_dir_varibad,
    args_cheetah_vel_oracle,
    ...
    for i in range(args.num_agents):
        ...
        print(
            "Evaluating sample of agent {} of {}, named: {} ...\n\n".format(
                agent_num, num_agents, agent_dict["model_name"]
            )
        )
...
def calculate_for_policy(vae_enc, args):
    ...
    signals, noises = [], []
    signal_noises, antisignal_noises = (
        [],
        [],
    )  # A trajectory of combines signal and noise, or -signal and noise. Used if args.cor.
    ...
    for _ in range(args.num_runs_per_agent):
        if not args.noise:  # if not just noises, need signals
            signal_observations = np.zeros(
                (args.length_of_decay, vae_enc.agg.input_size)
            )
            if args.strong:  # signal randomly and repeatedly
                for i in range(
                    len(signal_observations)
                    - int(args.strong_block * len(signal_observations))
                ):
            ...
            signals.append(
                get_states_from_obs(
                    ...
                    signal_observations,
                    args.grad,
                    param_grad=args.param_grad,
                    inputs_grad=args.inputs_grad,
                )
            )
        if not args.signal:  # if not just signals, need noises
            ...
            noise_observations = [
                np.random.uniform(-1, 1, vae_enc.agg.input_size) * ones_input
                for _ in range(args.length_of_decay)
            ]
            noises.append(
                get_states_from_obs(
                    vae_enc,
        ...
        if args.cor or args.tdist or args.lcc:
            signal_noise_obs = [
                np.random.choice([1, -1], p=[0.5, 0.5]) * ones_input
                for _ in range(args.length_of_decay)
            ]
            antisignal_noise_obs = [
                np.random.choice([1, -1], p=[0.5, 0.5]) * ones_input
                for _ in range(args.length_of_decay)
            ]
            ...
            antisignal_noises.append(
                get_states_from_obs(vae_enc, antisignal_noise_obs, False)
            )
        if args.perm_diff:
            perm_noise = []
            for i in range(args.length_of_decay):
                perm_obs_so_far = np.random.permutation(
                    np.array(noise_observations[: i + 1])
                )
    ...
    if args.normalized_norm:
        ...
        norms = np.sum(np.array(trajectory) ** 2, axis=-1) ** 0.5
        ...
        to_return = (
            cor(np.array(signal_noises), np.array(antisignal_noises)),
            steps[0],
        )
    elif args.tdist:
        signal_noises, steps = zip(*signal_noises)
        antisignal_noises, _ = zip(*antisignal_noises)
        to_return = (
            tdist(np.array(signal_noises), np.array(antisignal_noises)),
            steps[0],
        )
    elif args.lcc:
        signal_noises, steps = zip(*signal_noises)
        antisignal_noises, _ = zip(*antisignal_noises)
        to_return = (
            linear_classifier_confidence(
                np.array(signal_noises), np.array(antisignal_noises)
            ),
            steps[0],
        )
    else:
        (s, s_steps), (n, n_steps) = (
            expected_powers(signals, args),
            expected_powers(noises, args),
        )
        to_return = (s / n, s_steps)
        assert s_steps == n_steps, (s_steps, n_steps)
    steps_to_return = to_return[1]
    assert np.array(steps_to_return).shape == (args.length_of_decay,), (
        "Must have a single dimension of proper length"
    )
    return to_return


def get_states_from_obs(
    vae_enc, observations, grad, param_grad=False, inputs_grad=False, grad_step=1
):
    observations = torch.tensor(
        observations, dtype=torch.float, requires_grad=True
    ).reshape((len(observations), 1, len(observations[0])))
    ...
    if inputs_grad:  # Compute gradient of final output w.r.t. each input over time
        ...
        param_grad
    ):  # Compute gradient of output w.r.t. parameters of aggregator over time
        assert grad_step == 1, "grad_step !=1 is not tested."
        recurrent_out, state = vae_enc.agg(observations, state)
        ...
        for i in steps_to_return:
            ...
            g = torch.cat(
                [
                    (
                        torch.zeros(param.shape) if param_grad is None else param_grad
                    ).reshape((-1,))
                    for param_grad, param in zip(g, params)
                ]
            )
            grads.append(g)
        vals_to_return = torch.stack(grads, dim=0).squeeze().detach().numpy()
...
def cor(signal_noises, antisignal_noises):
    ...
    covariance = np.mean(
        (signal_noises - mean_signal) * (antisignal_noises - mean_antisignal), axis=0
    )
    correlation = covariance / (std_signal * std_antisignal)

    return np.mean(
        1 - correlation, axis=1
    )  # Compute 1 - correlation, and average over features
...
def perm_diff(noises, perm_noises, eps=1e-30):
    ...
    normalized_noises = noises / np.clip(
        np.linalg.norm(noises, axis=-1, keepdims=True), 1e-30, None
    )
    normalized_perm_noises = perm_noises / np.clip(
        np.linalg.norm(perm_noises, axis=-1, keepdims=True), 1e-30, None
    )

    diffs = np.linalg.norm(
        normalized_noises - normalized_perm_noises, axis=-1, keepdims=False
    )
...
def tdist(signal_noises, antisignal_noises, k=2):
    ...
    antisignal_tsne = antisignal_tsne.reshape(
        antisignal_noises.shape[0], antisignal_noises.shape[1], k
    )
...
def linear_classifier_confidence(signal_noises, antisignal_noises):
    ...
    y = np.concatenate(
        [np.ones(signal_flat.shape[0]), np.zeros(antisignal_flat.shape[0])]
    )
    ...
    signal_with_bias = np.hstack(
        [signal_flat_normalized, np.ones((signal_flat_normalized.shape[0], 1))]
    )
    confidences = signal_with_bias @ params

    # Reshape confidences back to original shape (samples, timesteps)
    confidences_reshaped = confidences.reshape(
        signal_noises.shape[0], signal_noises.shape[1]
    )
...
def expected_powers(runs, args):
    ...
    assert power.shape[0] == args.num_runs_per_agent, (
        power.shape[0],
        args.num_runs_per_agent,
    )
    assert power.shape[1] == length_of_runs, (power.shape[1], length_of_runs)
    expected_power = np.mean(power, axis=0)
    assert expected_power.shape == (length_of_runs,), (
        expected_power.shape,
        (length_of_runs,),
    )
...
def get_all_results(args):
    ...
    for agent_num, m_dict in enumerate(models):
        ...
        if args.dummy:
            ...
            grads = calculate_quantity(
                agent_dict, args, agent_num=agent_num + 1, num_agents=len(models)
            )
            snr_args = Namespace(**vars(args))
            snr_args.grad = False
            snr_args.signal = False
            snr_args.noise = False
            snrs = calculate_quantity(
                agent_dict, snr_args, agent_num=agent_num + 1, num_agents=len(models)
            )
            for grad_run, snr_run in zip(grads, snrs):
                grad_vals, grad_steps = grad_run
                snr_vals, snr_steps = snr_run
                assert snr_steps == tuple(range(args.length_of_decay)), (
                    snr_steps,
                    tuple(range(args.length_of_decay)),
                )
            ...
            agent_dict[AGENT_KEY] = calculate_quantity(
                agent_dict, args, agent_num=agent_num + 1, num_agents=len(models)
            )
        if args.final:
            agent_dict[AGENT_KEY] = [
                ([vals[-1]], [steps[-1]]) for (vals, steps) in agent_dict[AGENT_KEY]
            ]
            if AGENT_KEY == "snr":
                print(
                    "final SNRs for agent {}:".format(model_name),
                    agent_dict["snr"],
                    "\n",
                )
...
def make_plot(results_dicts, args):
    ...
    for agent_dict in results_dicts:
        if agent_dict["model_name"] not in MODEL_ORDER:
            print(
                "Warning, model, {}, not in model order.".format(
        ...
        if agent_dict["model_name"] not in COLOR_PALETTE:
            print(
                "Warning: model name,",
                agent_dict["model_name"],
                "not in COLOR_PALETTE. It will be added with a random color.",
            )
            h = zlib.adler32(
                agent_dict["model_name"].encode()
            )  # cannot use python hash(), since it is stochastic
            r = np.random.default_rng(
                seed=abs(h)
            )  # use hash of model name as seed so colors are reproducible between runs
            COLOR_PALETTE[agent_dict["model_name"]] = tuple(0.75 * r.uniform(size=(3,)))
    ...
    if args.final or args.sort:
        model_order = sorted(
            model_to_final_perfs.keys(), key=lambda m: np.mean(model_to_final_perfs[m])
        )
    else:
    ...
    if args.final:
        ...
        plot = sns.lineplot(
            ...
            estimator="mean",
            hue="Model",
            hue_order=reversed(model_order),
            data=pd.DataFrame(data),
            ci=args.ci,
            palette=palette,
        )
    ...
    plt.savefig(
        OUT_PATH_PNG, dpi=1000, bbox_inches="tight"
    )  # useful for much smaller plots in final paper
...
def main():
    os.environ["CUDA_VISIBLE_DEVICES"] = (
        "-1"  # Should run faster this way, since not batching
    )
    ...
    parser.add_argument(
        "--scale",
        type=float,
        help="the magnitude of each component of the signal or noise",
        default=1,
    )
    parser.add_argument(
        "--exclude",
        type=str,
        help="model names to exclude seperated by commas with no spaces",
        default=default_exclude,
    )
    parser.add_argument(
        "--extra_exclude",
        type=str,
        help="model names to exclude in addition to exclude (which has defaults)",
        default="",
    )
    parser.add_argument(
        "--ci", type=int, help="confidence interval for plot", default=68
    )
    parser.add_argument(
        "--num_agents",
        type=int,
        help="number of samples (initialized agents) to use when calculating SNR",
        default=3,
    )
    parser.add_argument(
        "--num_runs_per_agent",
        type=int,
        help="number of runs of an agent to estimate a single SNR",
        default=None,
    )
    parser.add_argument(
        "--length_of_decay",
        type=int,
        help="number of timesteps to feed in no stimulus (or noise)",
        default=None,
    )
    parser.add_argument(
        "--dummy",
        action="store_true",
        help="Whether to return a dummy DNR for testing",
        default=False,
    )
    parser.add_argument(
        "--signal",
        action="store_true",
        help="Whether to display plot of signal over time instead",
        default=False,
    )
    parser.add_argument(
        "--noise",
        action="store_true",
        help="Whether to display plot of noise over time instead",
        default=False,
    )
    parser.add_argument(
        "--final",
        action="store_true",
        help="Whether to plot just final values as a bar plot",
        default=False,
    )
    parser.add_argument(
        "--strong",
        action="store_true",
        help="Whether the signal obs occurs repeatedly and radnomly",
        default=False,
    )
    parser.add_argument(
        "--strong_p",
        type=float,
        help="Probability of a signal per step if strong",
        default=0.01,
    )
    parser.add_argument(
        "--strong_block",
        type=float,
        help="Fraction of steps at end of strnog with no signal",
        default=0.2,
    )
    parser.add_argument(
        "--snr", action="store_true", help="Plot the SNR.", default=False
    )
    parser.add_argument(
        "--grad",
        action="store_true",
        help="Plot gradients with respect to initial input.",
        default=False,
    )
    parser.add_argument(
        "--param_grad",
        action="store_true",
        help="Plot gradients with respect to parameters over time.",
        default=False,
    )
    parser.add_argument(
        "--inputs_grad",
        action="store_true",
        help="Plot gradients of final output with respect to each input over time.",
        default=False,
    )
    parser.add_argument(
        "--snd",
        action="store_true",
        help="Plot signal noise difference (novel formula).",
        default=False,
    )
    parser.add_argument(
        "--perm_diff",
        action="store_true",
        help="Plot average difference in permutation encodings.",
        default=False,
    )
    parser.add_argument(
        "--invv",
        action="store_true",
        help="Plot inverse variance of noise.",
        default=False,
    )
    parser.add_argument(
        "--cor",
        action="store_true",
        help="Plot correlation of the signals over time.",
        default=False,
    )
    parser.add_argument(
        "--tdist",
        action="store_true",
        help="Plot distance of t-SNEover times instead of SNR",
        default=False,
    )
    parser.add_argument(
        "--lcc",
        action="store_true",
        help="Plot confidence of linear classifier over time.",
        default=False,
    )
    parser.add_argument(
        "--combo",
        action="store_true",
        help="Multiply grad plot by an SNR plot",
        default=False,
    )
    parser.add_argument(
        "--normalized_norm",
        action="store_true",
        help="Compute norm of the observations for one run, and normalize over time.",
        default=False,
    )
    parser.add_argument(
        "--norm", action="store_true", help="Compute output norms.", default=False
    )
    parser.add_argument(
        "--no_log", action="store_true", help="Do not plot in log scale", default=False
    )
    parser.add_argument(
        "--out_name",
        type=str,
        help="filename for output saved in plts.",
        default="analysis",
    )
    parser.add_argument(
        "--lower_ylim",
        type=float,
        help="Lower ylim for plot. E.g. 10e-30.",
        default=None,
    )
    parser.add_argument(
        "--upper_ylim", type=float, help="Upper ylim for plot. E.g. 1.", default=None
    )
    parser.add_argument(
        "--sort",
        action="store_true",
        help="Whether to sort by performance for the legend on lineplots",
        default=False,
    )
    parser.add_argument(
        "--skip_ST",
        action="store_true",
        help='Whether to skip models with "ST" in the name if calculating SNR, since straight through connections have the same SNR as without',
        default=False,
    )
    parser.add_argument(
        "--title", action="store_true", help="Show title", default=False
    )
    parser.add_argument(
        "--out_dir",
        type=str,
    ...
    if args.num_runs_per_agent == None:  # Defaults
        args.num_runs_per_agent = (
            1 if args.grad or args.inputs_grad or args.param_grad else 5
        )
    if args.length_of_decay == None:  # Defaults
        args.length_of_decay = 2000 if args.strong else 100
    assert not (args.signal and args.noise), (
        "--signal and --noise are mutually exclusive"
    )
    if args.norm or args.grad or args.param_grad or args.inputs_grad:
        assert args.noise or args.signal, (
            "--grad must be evaluated in signal or noise env"
        )
    assert not (args.combo and not args.grad), "--combo is a modification of grad plot"
    assert (
        int(args.norm)
        + int(args.normalized_norm)
        + int(args.snr)
        + int(args.snd)
        + int(args.invv)
        + int(args.cor)
        + int(args.grad)
        + int(args.param_grad)
        + int(args.inputs_grad)
        + int(args.tdist)
        + int(args.perm_diff)
    ) == 1, "Exactly one quantity to measure must be specified."
