...
def get_mean_grad_norm(to_diff, input_batch):
    dv_di_batch = torch.autograd.grad(
        to_diff, input_batch, create_graph=False, allow_unused=False, retain_graph=True
    )[0]
...
def get_samples_by_task(sample):
    ...
    assert len(task_id_for_item) == len(task_batch), (
        len(task_id_for_item),
        len(task_batch),
    )
    ...
    for task_id in task_ids:
        bool_mask = [
            (item_id == task_id) for item_id in task_id_for_item
        ]  # find all elements in sample for this task_id=specific sample
        samples.append(
            tuple((None if tensor is None else tensor[bool_mask]) for tensor in sample)
        )
...
class PPO:
    ...
    def set_vae_optimizer(self, optimiser_vae):
        ...
        if (
            self.policy_anneal_lr
            and hasattr(self.args, "rlloss_through_encoder")
            and self.args.rlloss_through_encoder
        ):
    ...
    def update(
        ...
        if num_warmups is not None:
            ...
            hyper_reset = self.args.hyper_onehot_hard_reset and (
                self.args.hyper_onehot_num_warmup is not None
            )
            ti_reset = self.args.ti_coeff is not None and self.args.ti_hard_reset
            freeze_vae_during_warmup = (
                hyper_reset or ti_reset or self.args.freeze_vae_during_warmup
            ) and warming_up
            ...
            for sample in data_generator:
                ...
                if self.args.compute_grad_info:
                    ...
                    if latent_batch is not None:
                        latent_grad_norm += get_mean_grad_norm(
                            values.sum(), latent_batch
                        )
                ...
                if (
                    (encoder is not None)
                    and rlloss_through_encoder
                    and not freeze_vae_during_warmup
                ):
                    nn.utils.clip_grad_norm_(
                        encoder.parameters(), self.args.policy_max_grad_norm
                    )
        ...
        if (
            (not rlloss_through_encoder)
            and (self.optimiser_vae is not None)
            and not freeze_vae_during_warmup
        ):
            # Note: if rlloss_through_encoder, vae loss is already computed: it is computed above in self.get_loss
            if (
                self.args.ti_coeff is not None
                and self.args.ti_target == "base_net"
                and not self.args.ti_target_stop_grad
            ):
            ...
            for _ in range(self.args.num_vae_updates):
                assert not self.args.disable_decoder, (
                    "Decoder loss needed to train task encoder."
                )
                compute_vae_loss(
                    update=True,
                    encoded_task_func=self.get_encoded_task_func(),
                    policy_optimiser=policy_opt_to_vae,
                )
    ...
    def get_encoded_task_func(self):
        ...
        if (
            self.args.ti_coeff is not None
        ):  # pass encoded_task_func if doing task inference
            assert self.args.decode_task, (
                "When using task inference with learned embedding (args.ti_coeff) and vae, please set args.decode_task to True"
            )
            if self.args.ti_target == "task":

                def task_target_encoding(task_to_encode):
                    state_shape = task_to_encode.shape[:-1] + (
                        self.actor_critic.dim_state,
                    )
                    _, _, _, encoded_task_batch, _, _ = (
                        self.actor_critic.get_encoded_inputs(
                            torch.zeros(state_shape).to(device),
                            None,
                            None,
                            task_to_encode,
                        )
                    )  # Note state should really be ignored here
    ...
    def get_loss(
        ...
        if (
            include_ti_loss
            and (self.args.decode_task or self.args.ti_coeff is not None)
            and rlloss_through_encoder
        ):  # Note: if not rlloss_through_encoder, this loss will be computed in vae_loss() instead
            loss = self.add_ti_loss(
                loss, state_batch, latent_batch, belief_batch, task_batch, actions_batch
            )
        ...
        if return_all_losses:
            if (
                "one_hot_task" in info
            ):  # return the one-hot version policy used so we can calculate its gradient
    ...
    def add_ti_loss(
        self, loss, state_batch, latent_batch, belief_batch, task_batch, actions_batch
    ):
        # add loss for task inference
        assert self.args.ti_target in ["task", "base_net"]
        if self.args.ti_coeff is not None:
            assert self.args.task_chance is not None, (
                "end-to-end task inference requires task multi-task training."
            )
        ...
        if self.args.ti_coeff is None:
            ...
            assert self.args.decode_task
            assert self.args.policy_latent_embedding_dim == task_batch.shape[-1], (
                self.args.policy_latent_embedding_dim,
                task_batch.shape[-1],
            )
            coeff = self.args.task_loss_coeff
            _, latent, _, _, _, _ = self.actor_critic.get_encoded_inputs(
                state_batch, latent_batch, belief_batch, task_batch
            )
            task_target = torch.detach(task_batch)
            latent_target = latent
        elif self.args.ti_target == "task":
            state, latent, belief, task, one_hot_task, info = (
                self.actor_critic.get_encoded_inputs(
                    state_batch, latent_batch, belief_batch, task_batch
                )
            )
            ...
            _, _, _, _, _, base_params_task = self.actor_critic.evaluate_actions(
                state=state_batch,
                latent=latent_batch,
                belief=belief_batch,
                task=task_batch,
            ...
            _, _, _, _, _, base_params_latent = self.actor_critic.evaluate_actions(
                state=state_batch,
                latent=latent_batch,
                belief=belief_batch,
                task=task_batch,
                ...
                training=True,
                update_num=self.update_index,
                force_latent_input=True,  # force latent
            )
    ...
    def get_grad_with_enc(self, to_diff, create_graph=True):
        ...
        g = torch.autograd.grad(
            to_diff, all_params, create_graph=create_graph, allow_unused=True
        )
        g = torch.cat(
            [
                ...
                .reshape((-1,))
                .to(device)
                for param_grad, param in zip(g, all_params)
            ]
        )
    ...
    def get_grad(self, to_diff, create_graph=True):
        ...
        g = torch.cat(
            [
                ...
                .reshape((-1,))
                .to(device)
                for param_grad, param in zip(g, self.actor_critic.parameters())
            ]
        )
        return g
