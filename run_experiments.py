...
def get_cmds(args, experiment_sets):
    ...
    for experiment_set in experiment_sets:
        ...
        for experiment in experiment_set["experiments"]:
            ...
            search_params = itertools.product(
                *experiment_set["search_arguments"].values()
            )
            for search_param_setting in search_params:
                ...
                named_search_params = {
                    list(experiment_set["search_arguments"].keys())[i]: param_value
                    for i, param_value in enumerate(search_param_setting)
                }
                ...
                cmdStr_2_mujoco[cmd_str] = (
                    str(experiment_set["mujoco_version"])
                    if "mujoco_version" in experiment_set
                    else args.default_mujoco_version
                )
    return cmds, cmdStr_2_argDict, cmdStr_2_mujoco


def is_match(
    exper_dict_1, exper_dict_2, override_logdir_1=None, override_logdir_2=None
):
    ...
    logdir1 = (
        exper_dict_1["results_log_dir"]
        if override_logdir_1 is None
        else override_logdir_1
    )
    logdir2 = (
        exper_dict_2["results_log_dir"]
        if override_logdir_2 is None
        else override_logdir_2
    )
    return (
        (
            os.path.basename(os.path.normpath(logdir1))
...
def filter_prior_success(
    cmds_lists, cmdStr_2_argDict, args, unmatched_warning=False, quiet=False
):
    if not quiet:
    ...
    for _, arg_dict in cmdStr_2_argDict.items():
        ...
        dir_name = os.path.basename(
            os.path.normpath(arg_dict["results_log_dir"])
        )  # within docker
        log_dir = os.path.join(log_dir, dir_name)
        exp_dirs_in_envs = glob.glob(
            os.path.normpath(log_dir) + "/*/*/"
        )  # log_base_dir/dir_name/env_name/*/
        exp_dirs.update(
            exp_dirs_in_envs
        )  # collection of all directories for individual runs
    ...
    for d in exp_dirs:
        is_done, has_conf = (
            os.path.exists(os.path.join(d, "DONE")),
        ...
        if is_done and has_conf:
            ...
            with open(json_path, "r") as json_config:
                success_dict_1 = json.load(
                    json_config
                )  # config from successful experiments
                successful_config_dir_tup.append((success_dict_1, d))
        else:
            failed_dirs.append(d)
            if is_done:
                print(
                    "Found directory DONE but no config. Likely SCP was unfinished. Please remove:",
                    d,
                )
    ...
    for cmd_list in cmds_lists:
        ...
        for cmd in cmd_list:
            ...
            for success_dict, suc_d in successful_config_dir_tup:
                if is_match(
                    arg_dict,
                    success_dict,  # override the logdir to reflect where we found it...
                    override_logdir_2=os.path.dirname(
                        os.path.dirname(os.path.normpath(suc_d))
                    ),
                ):
                    matches.append(suc_d)
                    if suc_d in unmatched_successful_dirs:
                        unmatched_successful_dirs.remove(
                            suc_d
                        )  # This successful prior exp has a matching cmd
            if matches:
                # too much to print:
                # if not quiet:
...
def plot(args, free_cpus, exps_names=None, async_procs=None):
    ...
    if args.plot_dir is None:
        ...
        env_dirs = [
            ...
            for e in exps_names
            for p in glob.glob(
                os.path.normpath(args.log_base_dir_here)
                + "/"
                + e
                + "/logs_"
                + args.plot_env
    ...
    for dir_to_plot in env_dirs:
        for tag in args.plot_tags:
            ...
            set_dir, env_name = (
                os.path.basename(os.path.dirname(dir_to_plot)),
                os.path.basename(dir_to_plot),
            )
            dir_to_plot = os.path.join(
                args.docker_log_base_dir, set_dir, env_name
            ...
            tag_str = tag_str.replace(
                "/", "_"
            )  # / in out path will mess up saving in the plotting script
            plot_out_path = os.path.join(
                plot_dir, set_dir + "_" + env_name + tag_str
            ).replace("_logs", "")
            cmd_str = (
                args.docker_util_cmd
                + " "
                + args.viz_python_cmd
                + " "
                + dir_to_plot
            ...
            if (
                env_name == "logs_T-LN-P1-v0" and set_dir == "HML1"
            ):  # override specific to plotting this experiment
                cmd_str += " --step " + "750"
            ...
            try:
                ...
                if async_procs is None:
                    plot_output = subprocess.check_output(
                        cmd_str, stderr=subprocess.STDOUT, shell=True
                    )
                else:
                    async_procs.append(
                        subprocess.Popen(cmd_str, stderr=subprocess.STDOUT, shell=True)
                    )
                    plot_output = ""
...
def filter_and_clean(cpu_cmds, gpu_cmds, cmdStr_2_argDict, args):
    ...
    # filter and clean
    (cpu_cmds, gpu_cmds), _, failed_dirs, duplicate_dirs = filter_prior_success(
        [cpu_cmds, gpu_cmds],
        cmdStr_2_argDict,
        args,
...
def get_cpu_and_gpu_cmds(args):
    ...
    if args.force_gpu:
        ...
        gpu_cmds, gpuCmdStr_2_argDict, gpuCmdStr_2_mujoco = get_cmds(
            args, CPU_EXPERIMENT_SETS + GPU_EXPERIMENT_SETS
        )
    else:
        cpu_cmds, cpuCmdStr_2_argDict, cpuCmdStr_2_mujoco = get_cmds(
            args, CPU_EXPERIMENT_SETS
        )
        gpu_cmds, gpuCmdStr_2_argDict, gpuCmdStr_2_mujoco = get_cmds(
            args, GPU_EXPERIMENT_SETS
        )
...
def remove_term_procs(
    devID_2_procs,
    pid_2_cmd,
    cmdStrs_2_stopTime,
    cmdStr_2_argDict,
    active_cmds_here_file,
    async_procs,
    args,
):
    ...
    for dev_id, procs in list(devID_2_procs.items()):
        ...
        for p in procs:
            if p.poll() is None:
                ...
                print(
                    "\rExp terminated:",
                    pretty_str(cmdStr_2_argDict[" ".join(cmd_terminated)]),
                )
                print("\ri.e. cmd:", " ".join(cmd_terminated))
                _, matched_dirs, _, _ = filter_prior_success(
                    [[cmd_terminated]], cmdStr_2_argDict, args, quiet=True
                )
...
def update_active_cmds(
    add_cmds, active_cmds_here_file, cmdStr_2_argDict, args, async_procs=None
):
    for cmd in add_cmds:
        cmd_str = " ".join(cmd)
            ...
            if (len(procs) < procs_per_dev) and cmds_left:
                ...
                try:
                    ...
                    new_proc = subprocess.Popen(
                        full_cmd_str, stdout=stdout, stderr=subprocess.PIPE, shell=True
                    )
                    os.set_blocking(
                        new_proc.stdout.fileno(), False
                    )  # so we can read from these later
                    ...
                    print(
                        "\ri.e. exp:", pretty_str(cmdStr_2_argDict[" ".join(next_cmd)])
                    )
                    continue
...
def print_loop_updates(
    ...
    output_streams = [
        (proc.stdout, proc.pid) for proc in async_procs if proc.pid in pid_2_cmd
    ]
    output_streams += [
        (proc.stderr, proc.pid) for proc in async_procs if proc.pid in pid_2_cmd
    ]
    procs_printed = False
    for stream, pid in output_streams:
        ...
        while printed_lines < 100:
            ...
            if out_line.strip() != "":  # There is something to print for this line
                if (
                    (pid in pid_2_cmd)
                    and (" ".join(pid_2_cmd[pid]) in cmdStr_2_argDict)
                    and (printed_lines == 0)
                ):
    ...
    if (
        ((len(cmds_terminated) + len(cmds_failed)) > num_done_at_last_print)
        or procs_printed
        or (time() - last_print_time > 60)
    ):
        ...
        def print_cmd_updates(cmd):
            ...
            if cmd_str in cmdStr_2_startTime:
                if cmd_str in cmdStrs_2_stopTime:
                    up_time = "{:.2f}".format(
                        (cmdStrs_2_stopTime[cmd_str] - cmdStr_2_startTime[cmd_str])
                        / 3600
                    )
                else:
                    up_time = "{:.2f}".format(
                        (time() - cmdStr_2_startTime[cmd_str]) / 3600
                    )
            print("\r" + pretty_str(cmdStr_2_argDict[cmd_str]), flush=True)
            if up_time is not None:
                print("\r      Uptime {} hours, ".format(up_time) + info, flush=True)

        frac_done = len(cmds_terminated + cmds_failed) / len(
            cmds_terminated + running_cmds + cmds_remaining + cmds_failed
        )
        print("\r\nRunning... percent done: {}".format(100.0 * frac_done), flush=True)
    ...
    try:
        if print_info:
            ...
            exit_code = subprocess.call(
                cmd_str, stderr=subprocess.STDOUT, stdout=subprocess.DEVNULL, shell=True
            )
...
def build_dockerfile(docker_file, docker_dir):
    ...
    success = safe_exec_cmdstr_syncronous(
        "cd " + docker_dir + " && bash build.sh " + dockerfile_name,
        "build_dockers()",
        print_info=False,
    )
    if not success:
...
def build_dockers(args):
    ...
    docker_files = glob.glob(
        os.path.join(os.path.expanduser(args.docker_dir), "Dockerfile_*")
    )
...
def main():
    ...
    parser.add_argument(
        "EXPERIMENT_FILES",
        help="the filenames of experiment_sets to run (in ./experiment_sets/) with no commas and no extension",
        nargs="+",
    )
    parser.add_argument(
        "--docker_dir",
        help="specify a directories with docker imgs to build. Names must start with Dockerfile_",
        nargs="+",
        default=THIS_DIR,
    )
    parser.add_argument(
        "--cpu_cmd",
        help="a cmd for running with cpu argument, NOT detached",
        type=str,
        default=os.path.join(THIS_DIR, "run_cpu.sh") + " mujoco",
    )
    parser.add_argument(
        "--gpu_cmd",
        help="a cmd for running with gpu argument, NOT detached",
        type=str,
        default=os.path.join(THIS_DIR, "run_gpu.sh") + " mujoco",
    )
    parser.add_argument(
        "--default_mujoco_version",
        help="assumed mujoco_version if not specified",
        type=str,
        default="150",
    )
    parser.add_argument(
        "--docker_util_cmd",
        type=str,
        help="a cmd for running docker that does not detach and has psutil package",
        default=os.path.join(THIS_DIR, "run_any_dev.sh" + " mujoco150"),
    )
    parser.add_argument(
        "--python_cmd", type=str, default="python " + os.path.join(THIS_DIR, "main.py")
    )
    parser.add_argument(
        "--viz_python_cmd",
        type=str,
        default="python " + os.path.join(THIS_DIR, "visualize_runs.py"),
    )
    parser.add_argument(
        "--cpu_free",
        type=str,
        default="0-0",
        help="None or the range of cpus free, e.g. 0-10, inclusive",
    )  # required if cpu experiments
    parser.add_argument(
        "--gpu_free",
        type=str,
        default="0-0",
        help="None or the range of gpus free, e.g. 0-3, inclusive",
    )  # required if gpu experiments
    parser.add_argument(
        "--experiments_per_cpu",
        type=int,
        default=1,
        help="the number of cpu only experiments per cpu, e.g. 3",
    )
    parser.add_argument(
        "--experiments_per_gpu",
        type=int,
        default=5,
        help="the number of gpu experiments per gpu, e.g. 3",
    )
    parser.add_argument(
        "--force_gpu", help="move all experiments to gpu", action="store_true"
    )
    parser.add_argument(
        "--cpu_measure_sec",
        type=int,
        default=10,
        help="the number of seconds over which to estimate cpu usage",
    )
    parser.add_argument(
        "--log_base_dir_here",
        type=str,
        help="the base log dir on machine",
        default=os.path.join(THIS_DIR, "data"),
    )
    parser.add_argument(
        "--docker_log_base_dir",
        type=str,
        help="the same base log dir, but from within docker",
        default=os.path.join(THIS_DIR, "data"),
    )
    parser.add_argument(
        "--plot_at_end",
        help="plot learning curves at end",
        type=bool_arg,
        default=False,
    )
    parser.add_argument(
        "--plot", help="only plot learning curves and nothing else", action="store_true"
    )
    parser.add_argument(
        "--prioritize",
        help="any experiment with this str in any part of the command will be run first (useful for slow commands)",
        type=str,
        default="_NoRec",
    )
    parser.add_argument(
        "--exclude",
        help="any experiment with this str will not be run",
        type=str,
        default=None,
    )
    parser.add_argument(
        "--shuffle", help="shuffle order of the experiments", action="store_true"
    )
    parser.add_argument(
        "--async_plot",
        help="plot asynchronously... faster but may make errors hard to read",
        type=bool_arg,
        default=True,
    )
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="relative dir in log_base_dir_here to plot. e.g. HMT1 If None, all will be plotted. Supports glob regex. * will print all. None will use dirs in experiments.",
    )
    parser.add_argument(
        "--plot_env",
        type=str,
        default="*",
        help="env name to plot. e.g. Hop-v0. If None, all envs will be plotted. Supports glob regex",
    )
    parser.add_argument(
        "--plot_exclude",
        help="model names to exclude from all plots separated by commas with no spaces",
        type=str,
        default="",
    )
    parser.add_argument(
        "--plot_include",
        help="model names to include in plots separated by commas with no spaces",
        type=str,
        default="",
    )
    parser.add_argument(
        "--low_y_lim",
        help="starting value for y axis as a string, e.g. '-5'",
        type=str,
        default=None,
    )
    parser.add_argument(
        "--up_y_lim",
        help="ending value for y axis as a string, e.g. '-5'",
        type=str,
        default=None,
    )
    parser.add_argument(
        "--ignore_not_done",
        help="run all experiments, ignoring those for which it is not clear if they have completed",
        action="store_true",
    )
    parser.add_argument(
        "--expand",
        help="use more devices as they become free",
        type=bool_arg,
        default=False,
    )
    parser.add_argument(
        "--devnull",
        help="pipe experiment output to devnull. useful for debugging runner output.",
        action="store_true",
    )
    parser.add_argument(
        "--indiv",
        help="plot individual runs instead of confidence interval",
        action="store_true",
    )
    parser.add_argument(
        "--all_lr", help="plot all lrs, not best over lrs", action="store_true"
    )
    parser.add_argument(
        "--lr_keys",
        type=str,
        help="The key for the tuned learning rate. Plotting only supports search over one lr.",
        default="lr_policy lr_vae",
    )
    parser.add_argument(
        "--plot_tags",
        nargs="+",
        default=[None],
        help="tag to plot. If a tag is None, default from plt script",
    )
    parser.add_argument(
        "--final_r",
        help="plot order by final return instead of average return over training",
        action="store_true",
    )
    parser.add_argument(
        "--no_legend", help="no legend for plotting", action="store_true"
    )
    parser.add_argument(
        "--log_scale", help="plot in log scale", type=bool_arg, default=False
    )
    parser.add_argument("--smooth", help="smooth sz for plots", type=str, default=None)
    parser.add_argument(
        "--step", help="max step for plotting (k)", type=str, default=None
    )
    parser.add_argument(
        "--shift_warmups",
        help="Shift plots with pre-compute specified in the name.",
        action="store_true",
    )
    parser.add_argument(
        "--metric_tag", help="metric tag for plotting", type=str, default=None
    )
    ...
    if args.plot:
        ...
        exps_names = {
            exp["dir_name"] for exp in CPU_EXPERIMENT_SETS + GPU_EXPERIMENT_SETS
        }
        plot(
            args,
            free_cpus,
            exps_names=exps_names,
            async_procs=[] if args.async_plot else None,
        )
        exit()
    ...
    cpuID_2_procs, gpuID_2_procs = (
        {},
        {},
    )  # Storage for later, will be good to print at exit
    ...
    def at_exit_handler():
        ...
        print(
            "pids killed on each free cpu:",
            [[p.pid for p in procs] for procs in list(cpuID_2_procs.values())],
        )
        print(
            "pids killed on each free gpu:",
            [[p.pid for p in procs] for procs in list(gpuID_2_procs.values())],
        )
        with open(active_cmds_here_file, "w") as f:
            pass
    ...
    if cpu_cmds:
        assert free_cpus or args.expand, (
            "Must have free CPU or expand=True if running on CPU"
        )
    if gpu_cmds:
        assert free_gpus or args.expand, (
            "Must have free GPU or expand=True if running on GPU"
        )
    ...
    if args.prioritize:  # keep in mind that cmds are popped from the end!
        ...
        cpu_cmds = [
            cmd for cmd in cpu_cmds if not is_prioritized(cmd, args)
        ] + cpu_to_prioritize
        gpu_cmds = [
            cmd for cmd in gpu_cmds if not is_prioritized(cmd, args)
        ] + gpu_to_prioritize
        print(
            "Number of commands prioritized:",
            len(cpu_to_prioritize) + len(gpu_to_prioritize),
        )
    ...
    while cpu_cmds or gpu_cmds or running_cmds:
        ...
        num_done_at_last_print = print_loop_updates(
            cmds_terminated,
            cmds_failed,
            running_cmds,
            cpu_cmds + gpu_cmds,
        ...
        more_terminated, more_failed = remove_term_procs(
            cpuID_2_procs,
            pid_2_cmd,
            cmdStrs_2_stopTime,
            cmdStr_2_argDict,
            active_cmds_here_file,
            corruptable_async_procs,
            args,
        )
        ...
        cpu_cmds, started_a_cpu_cmd = fill_devices(
            cpu_cmds,
            args.experiments_per_cpu,
            cpuID_2_procs,
            args.cpu_cmd,
            pid_2_cmd,
            pid_2_devID,
        ...
        gpu_cmds, started_a_gpu_cmd = fill_devices(
            gpu_cmds,
            args.experiments_per_gpu,
            gpuID_2_procs,
            args.gpu_cmd,
            pid_2_cmd,
            pid_2_devID,
    ...
    if args.plot_at_end:
        ...
        plot(
            ...
            free_cpus,
            exps_names=exps_names,
            async_procs=async_procs if args.async_plot else None,
        )
