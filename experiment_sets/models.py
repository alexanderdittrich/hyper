...
def onehot_task_chance(exp_dict, chance):
    ...
    new_dict["exp_label"] = (
        "TaskChance" + str(int(100 * chance)) + "_" + exp_dict["exp_label"]
    )
...
def full_task_chance(exp_dict, chance):
    new_dict = exp_dict.copy()
    new_dict["exp_label"] = (
        "FullTaskChance" + str(int(100 * chance)) + "_" + exp_dict["exp_label"]
    )
...
def ti_naive(exp_dict, weight, task_sz):
    new_dict = exp_dict.copy()
    new_dict["exp_label"] = (
        "TINaive" + str(int(100 * weight)) + "_" + new_dict["exp_label"]
    )
...
def div_lr(
    exp_dict, factor, adjust_vae_lr=True
):  # useful to adjust LR of experiment to create new one
    new_dict = exp_dict.copy()
...
def make_RL2_fixedLR(
    exp_dict, lr=0.0007
):  # useful to turn VariBAD into RL2 with fixed LR. Note: still variational
    new_dict = exp_dict.copy()
...
def make_pol_size(size, exp_dict):
    ...
    assert size in hn_pol_sizes, "Given policy size {} not in defined set {}".format(
        size, list(hn_pol_sizes.keys())
    )
    new_dict = exp_dict.copy()
...
def make_hn_size(size, exp_dict):
    hn_sizes = {"S": "16", "M": "32"}
    assert size in hn_sizes, "Given hypernetwork size {} not in defined set {}".format(
        size, list(hn_sizes.keys())
    )
    new_dict = exp_dict.copy()
...
def check_exps(exp_sets):
    ...
    for experiment_set in exp_sets:
        ...
        assert not set_name in set_names_so_far, "duplicate set name: {}".format(
            set_name
        )
        # check for duplicates within set
        set_names_so_far.add(set_name)
        experiment_labels = [
            exp_dict["exp_label"] for exp_dict in experiment_set["experiments"]
        ]
        labels_so_far = set()
        for l in experiment_labels:
            assert not l in labels_so_far, (
                "exp_label duplicate found for {} in set {}".format(
                    l, experiment_set["set_name"]
                )
            )
            labels_so_far.add(l)
...
def set_frames(exp_dict, num_frames):
    ...
    new_dict["exp_label"] = (
        new_dict["exp_label"] + "_" + format(num_frames, ".0e") + "frames"
    )
    return new_dict
...
FiLM = {
    ...
    "init_hyper_for_policy": False,
}
FiLM_init_b = {
    "exp_label": "FiLM_init_b",
    "use_film": True,
    "use_hypernet": True,
    "hypernet_head_bias": True,
    "policy_layers_hyper": "1 1 1",
...
HyperNet_init_wb = {
    "exp_label": "HyperNet_init_wb",
    "use_hypernet": True,
    "hypernet_head_bias": True,
    "policy_layers_hyper": "1 1 1",
    "init_adjust_weights": True,
    "init_adjust_bias": True,
}
HyperNet_init_w = {
    "exp_label": "HyperNet_init_w",
    "use_hypernet": True,
    "hypernet_head_bias": True,
    "policy_layers_hyper": "1 1 1",
    "init_adjust_weights": True,
    "init_adjust_bias": False,
}
HyperNet_init_b = {
    "exp_label": "HyperNet_init_b",
    "use_hypernet": True,
    "hypernet_head_bias": True,
    "policy_layers_hyper": "1 1 1",
    "init_adjust_weights": False,
...
HyperNet_no_init = {
    "exp_label": "HyperNet_no_init",
    "use_hypernet": True,
    "hypernet_head_bias": True,
    "policy_layers_hyper": "1 1 1",
    "init_hyper_for_policy": False,
}
HyperNet_init_kaiming = {
    "exp_label": "HyperNet_kaiming",
    "use_hypernet": True,
    "hypernet_head_bias": True,
    "policy_layers_hyper": "1 1 1",
    "init_hyper_for_policy": False,
    "policy_initialisation": "kaiming",
}
HyperNet_init_normc = {
    "exp_label": "HyperNet_normc",
    "use_hypernet": True,
    "hypernet_head_bias": True,
    "policy_layers_hyper": "1 1 1",
    "init_hyper_for_policy": False,
...
HyperNet_init_HFI = {
    "exp_label": "HyperNet_init_HFI",
    "use_hypernet": True,
    "hypernet_head_bias": True,
    "policy_layers_hyper": "1 1 1",
    "policy_initialisation": "hfi",
}

RL2_HyperNet_init_b = {
    "exp_label": "RL2_HyperNet_init_b",
    "disable_decoder": True,
    "norm_latent_for_policy": False,
    "rlloss_through_encoder": True,
    "use_hypernet": True,
...
VI_HN_noInit = set_name(
    make_pol_size("XL", make_varibad(HyperNet_no_init)), "VI_HN_noInit"
)

TI_Naive = set_name(ti_naive(make_pol_size("XL", VARIBAD), 1.0, None), "TI_Naive")
...
RNN = set_name(
    state_sz(make_pol_size("XL", make_nonVar(RL2)), None), "RNN"
)  # RNN baseline
...
RNN_HN_Kaiming = set_name(
    make_pol_size("XL", make_nonVar(make_RL2(HyperNet_init_kaiming))), "RNN+HN_Kaiming"
)
...
TI = set_name(
    ti(
        num_full_warmups(full_task_chance(make_pol_size("XL", VARIBAD), 0), 563),
        1.0,
        base_net=False,
        stopgrad=True,
        hard=True,
...
TI_Grid = set_name(
    ti(
        num_full_warmups(full_task_chance(make_pol_size("XL", VARIBAD), 0), 100),
        1.0,
        base_net=False,
        stopgrad=True,
        hard=True,
...
TI_pp_HN = set_name(
    ti(
        num_full_warmups(
            full_task_chance(make_pol_size("XL", make_varibad(HyperNet_init_b)), 0), 563
        ),
        1.0,
        base_net=False,
        stopgrad=True,
...
TI_pp_HN_Grid = set_name(
    ti(
        num_full_warmups(
            full_task_chance(make_pol_size("XL", make_varibad(HyperNet_init_b)), 0), 100
        ),
        1.0,
        base_net=False,
        stopgrad=True,
...
Multi_HN = set_name(
    make_multi(make_pol_size("XL", make_varibad(HyperNet_init_b))), "Multi+HN"
...
Max = set_name(
    noRNN(make_max(RNN_HN)), "Max"
)  # Note: This also has no skip connection; skip makes little sense without RNN
...
CNP = set_name(
    noRNN(make_avg(RNN_HN)), "CNP+HN"
)  # Note: Variational Inference Required for non-conditional NP
...
Transformer = set_name(
    make_transformer(RNN_HN, token_sz=16, mem_sz=None), "Transformer"
)

# Gaussian Aggregation, as in PEARL (Rakelly et al., 2019). Variational Inference Required.
PEARL = set_name(set_kl_weight(make_EtoE_VI(make_gauss(VI_HN)), 0.000001), "PEARL")
PEARL_vari = set_name(make_gauss(VI_HN), "PEARL-vari")
# Tuning of PEARL KL:
PEARL_kl1 = set_name(set_kl_weight(make_EtoE_VI(PEARL), 1), "PEARL-kl1")
PEARL_klp01 = set_name(set_kl_weight(make_EtoE_VI(PEARL), 0.01), "PEARL-klp01")
PEARL_klp0001 = set_name(set_kl_weight(make_EtoE_VI(PEARL), 0.0001), "PEARL-klp0001")
PEARL_klp000001 = set_name(
    set_kl_weight(make_EtoE_VI(PEARL), 0.000001), "PEARL-klp000001"
)
...
SplAgger_softmax = set_name(
    make_skip(set_softmax_temp(make_softmax(RNN_HN), 0.1)), "SplAgger-softmax"
)
SplAgger_wsoftmax = set_name(
    make_skip(set_wavg_temp(make_wavg(RNN_HN), 0.1)), "SplAgger-wsoftmax"
)
...
wsoftmax_tp1 = set_name(set_wavg_temp(noRNN(make_wavg(RNN_HN)), 0.1), "wsoftmax-tp1")
SOFTMAX_t1 = set_name(noRNN(set_softmax_temp(make_softmax(RNN_HN), 1)), "softmax-t1")
SOFTMAX_tp1 = set_name(
    noRNN(set_softmax_temp(make_softmax(RNN_HN), 0.1)), "softmax-tp1"
)
