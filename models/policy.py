...
class Policy(nn.Module):
    def __init__(
        ...
        def get_init(init_str):
            if init_str == "normc":
                init_ = lambda m: init(
                    m, init_normc_, lambda x: nn.init.constant_(x, 0), gain
                )
            elif init_str == "orthogonal":
                init_ = lambda m: init(
                    m, nn.init.orthogonal_, lambda x: nn.init.constant_(x, 0), gain
                )
            else:
                ...
                init_ = (
                    lambda m: init(
                        m,
                        lambda tensor, gain: nn.init.kaiming_uniform_(
                            tensor, mode="fan_in", nonlinearity=activation_function
                        ),
                        lambda x: nn.init.constant_(x, 0),
                        gain=1,
                    )
                )  # This is additional gain now, since kaiming_uniform_ calculates gain itself
            return init_
        ...
        if policy_initialisation.lower() != "hfi":
            ...
            if args.hypernet_input == "task_embed":
                ...
                if args.rlloss_through_encoder:
                    print(
                        "Warning, HFI expects normalized latent, but you cannot do that if passing rlloss_through_encoder"
                    )
                elif not self.args.norm_latent_for_policy:
                    print(
                        "Warning, HFI init will normalize latent input despite the argument setting"
                    )
        ...
        if (
            self.multinet
            or (args.pass_task_to_policy and args.pass_task_as_onehot_id)
            or (args.hyper_onehot_chance is not None)
        ):
            # need this env and info within to convert to one_hot
            self.dummy_env, _ = utl.make_env(args)
            assert hasattr(self.dummy_env, "task_to_id"), (
                "args.env_name must have task_to_id() function"
            )
            assert hasattr(self.dummy_env, "num_tasks"), (
                "args.env_name must have num_tasks attribute"
            )
        ...
        if self.multinet:  # multinet (separate net for each task) must have one-hot representation of input task
            ...
            self.pass_task_as_onehot_id = self.args.pass_task_as_onehot_id = (
                args.pass_task_as_onehot_id
            ) = False
            num_nets = dim_task
        ...
        if self.pass_state_to_policy and self.use_state_encoder:
            if self.multinet:
                for j in range(num_nets):  # make a state_encoder for each task network
                    self.net_2_st_layer[j] = utl.FeatureExtractor(
                        dim_state,
        ...
        if (
            self.use_hyper
        ):  # if using hypernet, we will instead pass task embedding to hypernet
            if args.hypernet_input == "task_embed":
                assert self.pass_task_to_policy, (
                    "The Hypernet needs the task passed to the policy in order to condition on it"
                )
                assert dim_task != 0, (
                    "Dimension of task cannot be 0. You may want to check that env has env.task_dim set properly."
                )
                hyper_in_dim = (
                    dim_task
                    if self.args.policy_task_embedding_dim is None
                    else self.args.policy_task_embedding_dim
                )
            else:
                assert args.hypernet_input == "latent", args.hypernet_input
                assert self.pass_latent_to_policy, (
                    "The Hypernet needs the latent passed to the policy in order to condition on it"
                )
                hyper_in_dim = (
                    dim_latent
                    if self.args.policy_latent_embedding_dim is None
                    else self.args.policy_latent_embedding_dim
                )
        ...
        self.dim_state, self.dim_latent, self.dim_belief, self.dim_task = (
            dim_state,
            dim_latent,
            dim_belief,
            dim_task,
        ...
        self.task_in_dim = (
            self.args.policy_task_embedding_dim if self.use_task_encoder else dim_task
        )

        if (
            args.task_chance is not None
        ):  # sometimes passing task, sometimes latent, but not both
            assert self.pass_task_to_policy
            assert self.pass_latent_to_policy
            assert self.args.policy_task_embedding_dim is not None, (
            ...
            curr_input_dim -= (
                self.args.policy_task_embedding_dim
            )  # doesn't matter which one here, because they must be the same size
        ...
        if self.use_film:
            assert self.use_hyper, (
                "If using FiLM, you must have use_hypernet=True, since we implement FiLM as a simplified Hypernet"
            )
        ...
        self.actor_layers = (
            nn.ModuleList()
        )  # actor layers, or None if this layer is produced by a hypernetwork
        ...
        if self.use_hyper:
            ...
            policy_layer_is_hyper = [
                True if is_hyper == "1" else False
                for is_hyper in args.policy_layers_hyper
            ]
            assert len(policy_layer_is_hyper) == (len(hidden_layers) + 1), (
                "--policy_layers_hyper must be specified for every layer and the policy head"
            )
            assert sum(policy_layer_is_hyper) >= 1, (
                "Must have some layers hyper if using hypernet. Adjust --policy_layers_hyper."
            )
            if args.init_hyper_for_policy:
                assert args.init_adjust_weights or args.init_adjust_bias, (
                    "Must adjust weights or bias in order to init hypernet to match original policy init"
                )
        for i in range(len(hidden_layers)):
            ...
            if layer_is_hyper:
                ...
                if self.use_film:  # hypernet only point-wise scales activations, but still produces full bias
                    assert args.hypernet_head_bias, (
                        "If using FiLM, you need a bias in the hypernet"
                    )
                    self.actor_layers.append(
                        init_(nn.Linear(curr_input_dim, hidden_layers[i], bias=False))
                    )  # generate base weights
                    self.critic_layers.append(
                        init_(nn.Linear(curr_input_dim, hidden_layers[i], bias=False))
                    )  # generate base weights
                    # define hypernet head for this layer
                    weight_sz = hidden_layers[
                        i
                    ]  # number of scaling factors to produce for this layer
                    ...
                    )  # size of weights required for this layer
                    bias_sz = hidden_layers[i]  # size of bias required for this layer
                # select init method for HyperNet
                if self.HFI:
                    bias_layer_init = HFI_bias_layer_init(
                        final_hyper_hidden_sz, gain=gain
                    )
                    weight_layer_init = HFI_weight_layer_init(
                        final_hyper_hidden_sz, curr_input_dim, gain=gain
                    )
                elif args.init_hyper_for_policy:
                    bias_layer_init = hyper_bias_layer_init()
                    weight_layer_init = hyper_weight_layer_init(
                        activation_function,
                        policy_initialisation,
                        final_hyper_hidden_sz,
                        curr_input_dim,
                        hidden_layers[i],
                        args.adjust_init_b_gain,
                        adjust_weights=args.init_adjust_weights,
                        adjust_bias=args.init_adjust_bias,
                        use_film=self.use_film,
                    )
                else:
                    bias_layer_init, weight_layer_init = init_, init_
                # Create Hyper Head layers
                self.hyper_final_to_actor_weight.append(
                    weight_layer_init(
                        nn.Linear(
                            final_hyper_hidden_sz,
                            weight_sz,
                            bias=args.hypernet_head_bias,
                        )
                    )
                )
                self.hyper_final_to_actor_bias.append(
                    bias_layer_init(
                        nn.Linear(
                            final_hyper_hidden_sz, bias_sz, bias=args.hypernet_head_bias
                        )
                    )
                )
                self.hyper_final_to_critic_weight.append(
                    weight_layer_init(
                        nn.Linear(
                            final_hyper_hidden_sz,
                            weight_sz,
                ...
                self.hyper_final_to_critic_bias.append(
                    bias_layer_init(
                        nn.Linear(
                            final_hyper_hidden_sz, bias_sz, bias=args.hypernet_head_bias
                        )
        ...
        if not self.multinet:
            assert len(self.actor_layers) == len(self.hyper_final_to_actor_weight), (
                len(self.actor_layers),
                len(self.hyper_final_to_actor_weight),
            )
        if not self.use_hyper:
            ...
            assert sum(
                [1 if f is not None else 0 for f in self.hyper_final_to_actor_weight]
            ) == sum(policy_layer_is_hyper[:-1])
        ...
        if self.head_is_hyper:
            ...
            if self.HFI:
                bias_layer_init = HFI_bias_layer_init(
                    final_hyper_hidden_sz, gain=1
                )  # linear so gain = 1
                weight_layer_init = HFI_weight_layer_init(
                    final_hyper_hidden_sz, hidden_layers[-1], gain=1
                )
            elif args.init_hyper_for_policy:
                bias_layer_init = hyper_bias_layer_init(
                    weight_for_uniform=torch.zeros((hidden_layers[-1], 1))
                )  # the bias was not initialized to 0 here in base critic. This tensor gives us the shape to recreate the base init.
                weight_layer_init = hyper_weight_layer_init(
                    "linear" if args.adjust_init_b_gain else activation_function,
                    "kaiming",
                    final_hyper_hidden_sz,
                    hidden_layers[-1],
                    1,  # the base critic used default nn.Linear, which uses kaiming
                    args.adjust_init_b_gain,
                    adjust_weights=args.init_adjust_weights,
                    adjust_bias=args.init_adjust_bias,
                    use_film=self.use_film,
                ...
                bias_layer_init, weight_layer_init = init_, init_
            # Create Hyper Head layers
            self.hyper_final_to_critic_linear_weight = weight_layer_init(
                nn.Linear(
                    final_hyper_hidden_sz, weight_sz, bias=args.hypernet_head_bias
                )
            )
            self.hyper_final_to_critic_linear_bias = bias_layer_init(
                nn.Linear(final_hyper_hidden_sz, bias_sz, bias=args.hypernet_head_bias)
            )
        else:
            if self.multinet:
                for j in range(num_nets):  # make a state_encoder for each task network
        ...
        if self.multinet:
            for j in range(num_nets):
                dist, dist_gain, num_outputs, _ = self.get_dist(
                    policy_initialisation,
                    action_space,
                    hidden_layers,
                    init_std,
            ...
            self.dist, dist_gain, num_outputs, actor_head_init = self.get_dist(
                policy_initialisation,
                action_space,
                hidden_layers,
                init_std,
        ...
        if self.head_is_hyper:
            ...
            if self.HFI:
                bias_layer_init = HFI_bias_layer_init(
                    final_hyper_hidden_sz, gain=dist_gain
                )  # use same gain for policy head as in other methods
                weight_layer_init = HFI_weight_layer_init(
                    final_hyper_hidden_sz, hidden_layers[-1], gain=dist_gain
                )
            elif args.init_hyper_for_policy:
                bias_layer_init = hyper_bias_layer_init()
                weight_layer_init = hyper_weight_layer_init(
                ...
                bias_layer_init, weight_layer_init = init_, init_
            # Create Hyper Head layers
            self.hyper_final_to_actor_linear_weight = weight_layer_init(
                nn.Linear(
                    final_hyper_hidden_sz, weight_sz, bias=args.hypernet_head_bias
                )
            )
            self.hyper_final_to_actor_linear_bias = bias_layer_init(
                nn.Linear(final_hyper_hidden_sz, bias_sz, bias=args.hypernet_head_bias)
            )
        else:
    ...
    def task_bn_params(self, task):
        ...
        action_shape = (
            task.shape[:-1] + (self.num_outputs,)
        )  # This shape may not be correct always, but should be ignore anyway since return_only_bn_params=True in call below
        base_params_task = self.evaluate_actions(
            state=torch.zeros(state_shape).to(device),
            latent=torch.zeros(latent_shape).to(device),
            belief=torch.zeros(belief_shape).to(device),
            task=task.to(device),
            action=torch.zeros(action_shape).to(device),
            return_action_mean=True,
            return_base_params=True,
            return_only_bn_params=True,
            return_info=False,
            training=True,
            update_num=0,
            force_task_input=True,  # force task
        )
    ...
    def should_overwrite_task(self, training, update_num):
        ...
        chance_argument = (
            self.args.task_chance
            if self.args.task_chance is not None
            else self.args.hyper_onehot_chance
        )
        num_warmup_argument = (
            self.args.task_num_warmup
            if self.args.task_num_warmup is not None
            else self.args.hyper_onehot_num_warmup
        )
        assert not (self.args.task_chance and self.args.hyper_onehot_chance), (
            "Cannot have both args.task_chance and args.hyper_onehot_chance"
        )
        assert not (self.args.task_num_warmup and self.args.hyper_onehot_num_warmup), (
            "Cannot have both args.task_num_warmup and args.hyper_onehot_num_warmup"
        )

        assert not (chance_argument is None and num_warmup_argument is not None), (
            "if args.hyper_onehot/task_num_warmup is specified, args.hyper_onehot/task_chance should be specified as well"
        )

        if (
            chance_argument is not None
        ) and training:  # just use one_hot task with some chance
            chance = (
                1
                if (num_warmup_argument is not None)
                and (update_num < num_warmup_argument)
                else chance_argument
            )  # set chance
    ...
    def forward_hyper(
        ...
        out_dim,
        policy_layers,
        hyper_final_to_policy_weight,
        hyper_final_to_policy_bias,
        hyper_final_to_policy_linear_weight,
        hyper_final_to_policy_linear_bias,
        store_base_params=False,
        replace_base_params_with_activations=False,
        one_hot_tasks=None,
        training=False,
        update_num=None,
    ):
        ...
        for i in range(len(self.hyper_layers)):
            ...
            use_softmax = (
                i == (len(self.hyper_layers) - 1)
                and self.args.hyper_softmax_temp is not None
            )
            af = (
                (lambda x: F.softmax(x / self.args.hyper_softmax_temp, dim=-1))
                if use_softmax
        ...
        task_overwrite = self.should_overwrite_task(training, update_num) and (
            self.args.hyper_onehot_chance is not None
        )
        if task_overwrite:
            assert one_hot_tasks is not None
            assert one_hot_tasks.shape == h_hyper.shape, (
                "Please make sure layer before hypernet head has dim "
                + str(one_hot_tasks.shape[-1])
            )
        ...
        h = inputs
        base_params = []
        for fc, hyper_w_head, hyper_b_head, next_sz in zip(
            policy_layers,
            hyper_final_to_policy_weight,
            hyper_final_to_policy_bias,
            self.hidden_layers,
        ):
            assert fc is not None or hyper_w_head is not None, (
                fc,
                hyper_w_head,
                hyper_b_head,
            )
            if fc is None:  # use hypernetwork
                assert hyper_w_head is not None and hyper_b_head is not None, (
                    fc,
                    hyper_w_head,
                    hyper_b_head,
                )
                ...
                weight = (
                    weight.reshape((next_sz, h.shape[-1]))
                    if batch_sz is None
                    else weight.reshape((batch_sz, next_sz, h.shape[-1]))
                )
                ...
                assert hyper_w_head is None and hyper_b_head is None, (
                    fc,
                    hyper_w_head,
                    hyper_b_head,
                )
        ...
        if (
            hyper_final_to_policy_linear_weight is None
        ):  # head of policy is not produced by hypernetwork
            ...
            final_weight = (
                final_weight.reshape((out_dim, h.shape[-1]))
                if batch_sz is None
                else final_weight.reshape((batch_sz, out_dim, h.shape[-1]))
            )
            final_bias = hyper_final_to_policy_linear_bias(h_hyper)
            if store_base_params:
                base_params.append(final_bias)
    ...
    def forward_actor(
        ...
        inputs,
        hyper_input=None,
        return_base_params=False,
        return_actor_activations=False,
        one_hot_tasks=None,
        training=False,
        update_num=None,
    ):
        ...
        if self.multinet:
            ...
            for i in range(len(self.net_2_actor_layers[0])):
                h = self.apply_multinet_layer(
                    [self.net_2_actor_layers[net][i] for net in range(self.num_nets)],
                    h,
                    hyper_input,
                )
                h = self.activation_function(h)
            ...
            if return_actor_activations:
                return activations
            return self.apply_multinet_layer(
                self.net_2_dist_layer, h, hyper_input, dist_layer=True
            )

        # Apply hyper and normal layers
        h, final_weight, final_bias, base_params, final_scaling = self.forward_hyper(
            inputs,
            hyper_input,
            self.num_outputs,
            self.actor_layers,
            self.hyper_final_to_actor_weight,
            self.hyper_final_to_actor_bias,
            self.hyper_final_to_actor_linear_weight,
            self.hyper_final_to_actor_linear_bias,
            store_base_params=return_base_params,
            replace_base_params_with_activations=return_actor_activations,
            one_hot_tasks=one_hot_tasks,
            training=training,
    ...
    def forward_critic(
        ...
        if self.multinet:
            ...
            for i in range(len(self.net_2_critic_layers[0])):
                h = self.apply_multinet_layer(
                    [self.net_2_critic_layers[net][i] for net in range(self.num_nets)],
                    h,
                    hyper_input,
                )
                h = self.activation_function(h)
            return self.apply_multinet_layer(
                self.net_2_critic_head_layer, h, hyper_input
            )
        ...
        h, final_weight, final_bias, base_params, final_scaling = self.forward_hyper(
            ...
            1,
            self.critic_layers,
            self.hyper_final_to_critic_weight,
            self.hyper_final_to_critic_bias,
            self.hyper_final_to_critic_linear_weight,
            self.hyper_final_to_critic_linear_bias,
            store_base_params=return_base_params,
            one_hot_tasks=one_hot_tasks,
            training=training,
            update_num=update_num,
        ...
        if (
            final_weight is None and final_bias is None and final_scaling is None
        ):  # Normal
    ...
    def apply_multinet_layer(self, multinet_layers, batch, tasks, dist_layer=False):
        ...
        normal_dist_outputs2 = (
            None  # needed just to keep track of extra params for normal dist
        )
        for task_id in range(self.num_nets):
            ...
            if type(net) is Categorical:
                ...
                for2 = for2.unsqueeze(0).expand(
                    len(batch), self.num_outputs
                )  # this is std for each action. it should be shared across batch
                if outputs is None:
    ...
    def get_encoded_inputs(self, state, latent, belief, task):
        ...
        if self.args.hyper_onehot_chance is not None:
            ...
            assert self.args.pass_task_as_onehot_id, (
                self.argsargs.pass_task_as_onehot_id
            )
            one_hot_task = torch.autograd.Variable(
                self.task_2_id(task), requires_grad=True
            )
        else:
            one_hot_task = None
        ...
        if self.pass_state_to_policy:
            ...
            if self.norm_state:
                state = (state - self.state_rms.mean) / torch.sqrt(
                    self.state_rms.var + 1e-8
                )
            if self.use_state_encoder:
    ...
    def forward(
        ...
        training=False,
        update_num=None,
        force_task_input=False,
        force_latent_input=False,
    ):
        state, latent, belief, task, one_hot_task, info = self.get_encoded_inputs(
        ...
        if self.use_hyper:
            if self.args.task_chance is not None:
                if (
                    self.should_overwrite_task(training, update_num) or force_task_input
                ) and not force_latent_input:
                    inputs = state
                    hyper_input = task
                else:
                    inputs = state
                ...
                task = torch.zeros(
                    ...
                    device
                )  # reset to 0, we already have one_hot task saved in one_hot_task
            ...
            inputs = torch.cat((state, latent, belief), dim=-1)
            hyper_input = (
                task  # Note: I call this hyper_input but really no hypernet used
            )
        elif (
            self.args.task_chance is not None
        ):  # Single network, No HN, but using task sometimes and latent sometimes
            if (
                self.should_overwrite_task(training, update_num) or force_task_input
            ) and not force_latent_input:
        ...
        if return_actor_activations:
            return self.forward_actor(
                ...
                return_actor_activations=True,
                one_hot_tasks=one_hot_task,
                training=training,
                update_num=update_num,
            )
        elif return_base_params:
            assert self.use_hyper
            value, c_base_params = self.forward_critic(
                inputs,
                hyper_input=hyper_input,
                return_base_params=True,
                one_hot_tasks=one_hot_task,
            ...
            dist, a_base_params = self.forward_actor(
                inputs,
                hyper_input=hyper_input,
                return_base_params=True,
                one_hot_tasks=one_hot_task,
    ...
    def get_dist(
        ...
        if action_space.__class__.__name__ == "Discrete":
            actor_head_init = (
                policy_initialisation
                if policy_initialisation == "kaiming"
                else "orthogonal"
            )  # leave kaiming alone, but otherwise use orthogonal
            dist_gain = 0.01
            ...
            dist = Categorical(
                ...
                num_outputs,
                dist_gain,
                create_params=self.use_film or not self.head_is_hyper,
            )
        elif action_space.__class__.__name__ == "Box":
            actor_head_init = (
                policy_initialisation if policy_initialisation == "kaiming" else "normc"
            )  # leave kaiming alone, but otherwise use normc
            dist_gain = 1.0
    ...
    def get_value(self, state, latent, belief, task, training=False, update_num=None):
        value, _ = self.forward(
            state, latent, belief, task, training=training, update_num=update_num
        )
        return value
    ...
    def update_rms(self, args, policy_storage):
        ...
        if self.pass_task_to_policy and self.norm_task:
            ...
            task = (
                self.task_2_id(task) if self.args.pass_task_as_onehot_id else task
    ...
    def task_2_id(self, task, one_hot=True):
        ...
        if len(one_hot_tasks.shape) == 2 and len(task.shape) == 1:
            ...
            assert len(one_hot_tasks.shape) == len(task.shape), (
                len(one_hot_tasks.shape),
                len(task.shape),
            )
    ...
    def evaluate_actions(
        ...
        force_task_input=False,
        force_latent_input=False,
        return_only_bn_params=False,
    ):
        if return_base_params:
            value, dist, base_params, info = self.forward(
                state,
                latent,
                belief,
                task,
                ...
                training=training,
                update_num=update_num,
                force_task_input=force_task_input,
                force_latent_input=force_latent_input,
            )
        else:
            value, dist, info = self.forward(
                state,
                latent,
                belief,
                task,
...
def HFI_bias_layer_init(
    final_hyper_hidden_sz, gain=1
):  # This gain should be passed as sqrt(2) for RelU
    variance = (
        1 / (2 * final_hyper_hidden_sz)
    )  # = 1/(2 * dl), since the rest of the terms cancel. (numerator will be 2 here if gain is passed for relu)
    bound = gain * np.sqrt(3 * variance)  # gain is sqrt(2) for RelU
    return lambda m: init(
        m,
        lambda w, gain: nn.init.uniform_(w, -bound, bound),
...
def HFI_weight_layer_init(
    final_hyper_hidden_sz, base_curr_input_dim, gain=1
):  # This gain should be passed as sqrt(2) for RelU
    variance = 1 / (
        2 * final_hyper_hidden_sz * base_curr_input_dim
    )  # = 1/(2 * dj * dk), since the rest of the terms cancel
    bound = gain * np.sqrt(3 * variance)  # gain is sqrt(2) for RelU
    return lambda m: init(
        m,
        lambda w, gain: nn.init.uniform_(w, -bound, bound),
...
def init_hyper_match(
    param,
    is_weight,
    policy_initialisation_str,
    hyper_layer_dim,
    input_dim,
    output_dim,
    adjust_init_b_gain,
    ...
    if policy_initialisation_str == "normc":
        ...
        policy_initialisation = (
            kaiming_uniform_with_gain
            if adjust_init_b_gain
            else kaiming_uniform_ignore_gain
        )
    ...
    if is_weight:
        ...
        assert param.shape == (input_dim * output_dim,), (
            param.shape,
    ...
    if is_weight:
        for col_indx in range(hyper_layer_dim):
            col = param[:, col_indx]
            policy_weight = col.reshape(
                (output_dim, input_dim)
            )  # This should be the shape expected for the init tensor inputs
...
def hyper_bias_layer_init(
    weight_for_uniform=None,
):  # weight_for_uniform needed to use default initialization from Linear class
...
def hyper_weight_layer_init(
    ...
    scale = 0.5 if adjust_weights and adjust_bias else None
    # define weight init
    if adjust_weights:
        if use_film:
            weight_init = (
                lambda w, gain: nn.init.constant_(w, 1 if scale is None else scale)
            )  # In FiLM, init all weights that produce scaling to 1 so that one-hot input produces scaling = 1. (.5 is shared with bias)
        else:
            weight_init = lambda w, gain: init_hyper_match(
                w,
                True,
                policy_initialisation_str,
                hyper_layer_dim,
    ...
    if adjust_bias:
        if use_film:
            bias_init = (
                lambda b: nn.init.constant_(b, 1 if scale is None else scale)
            )  # In FiLM, init all bias that produce scaling to 1 so that it produces scaling = 1. (.5 is shared with weights)
        else:
            gain_for_bias_also = True
            bias_init = lambda b, gain: init_hyper_match(
                b,
                False,
                policy_initialisation_str,
                hyper_layer_dim,
    ...
    if use_film:
        ...
        gain_for_bias_also = False
    return lambda m: init(
        m, weight_init, bias_init, override_gain, gain_for_bias_also=gain_for_bias_also
    )
...
class Categorical(nn.Module):
    def __init__(self, num_inputs, num_outputs, gain, create_params=True):
        ...
        init_ = lambda m: init(
            m, nn.init.orthogonal_, lambda x: nn.init.constant_(x, 0), gain=gain
        )

        self.linear = (
            init_(nn.Linear(num_inputs, num_outputs)) if self.create_params else None
        )

    def forward(
        self, x, weight=None, bias=None, scaling=None, return_dist_params=False
    ):
        if (weight is None) and (bias is None):  # Normal
            ...
            assert (weight is not None) and (bias is not None), (
                "Must have weight and bias for HyperNet"
            )
...
class DiagGaussian(nn.Module):
    def __init__(
        ...
        init_ = lambda m: init(
            m, init_normc_, lambda x: nn.init.constant_(x, 0), gain=gain
        )
        ...
        self.logstd = nn.Parameter(
            np.log(torch.zeros(num_outputs) + init_std)
        )  # Always created (easier to implement)
    ...
    def forward(
        ...
        if self.norm_actions_of_policy:
            if (
                self.unique_action_limits
                and torch.unique(self.action_low) == -1
                and torch.unique(self.action_high) == 1
            ):
                action_mean = torch.tanh(action_mean)
            else:
                # Note: this isn't tested
                action_mean = (
                    torch.sigmoid(action_mean) * (self.action_high - self.action_low)
                    + self.action_low
                )
