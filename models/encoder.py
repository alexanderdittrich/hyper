...
class RNNEncoder(nn.Module):
    def __init__(
        ...
        self.state_encoder = utl.FeatureExtractor(
            self.state_dim, self.state_embed_dim, F.relu
        )
        self.action_encoder = utl.FeatureExtractor(
            self.action_dim, self.action_embed_dim, F.relu
        )
        ...
        if self.args.full_transitions:
            self.agg_input_dim = (
                ...
                + (2 * self.state_embed_dim)
                + self.reward_embed_size
            )
        else:
            self.agg_input_dim = (
                self.action_embed_dim + self.state_embed_dim + self.reward_embed_size
            )
        ...
        self.agg = Aggregator(
            ...
            st_estimator=self.st_estimator,
            max_init_low=self.max_init_low,
            layers_before_enc=self.layers_before_gru,
            layers_after_enc=self.layers_after_gru,
    ...
    def prior(self, batch_size, sample=True):
        ...
        agg_out, _ = self.agg(
            torch.zeros((1, batch_size, self.agg_input_dim)).to(device), hidden_state
        )  # TODO: Should we replace the hidden state or start again at 0?
    ...
    def forward(
        ...
        if hidden_state is None:
            ...
            assert hidden_state.shape[-1] == self.agg.state_size, (
                hidden_state.shape,
                self.agg.state_size,
                self.agg.enc_state_size,
            )
        ...
        if return_prior:
            ...
            _, prior_mean, prior_logvar, prior_hidden_state = self.prior(
                actions.shape[1]
            )
        ...
        hs_next = self.state_encoder(
            states
        )  # these are next states, after actions and rewards
        hs_prev = self.state_encoder(
            prev_states
        )  # these are previous states, before actions and rewards
        # print("ha", ha)
        # print("hr", hr)
        # print("hs", hs_next)
        if self.args.full_transitions:
        ...
        agg_outputs, hidden_states = (
            None,
            None,
        )  # compute these1 step at a time to save on memory)
        if unpadded_lens is None:
            ...
            if seq_len != 1 and return_all_hidden:
                print(
                    "Warning: seq_len={}; requesting all hidden states for long sequences may cause memory issues.".format(
                        seq_len
                    )
                )
            if return_prior:
            ...
            for t in range(seq_len):
                ...
                agg_out, hidden_state = self.agg(
                    to_agg, hidden_state.clone(), return_all_hidden_states=False
                )
            ...
            unpadded_lens = [
                l - 1 for l in unpadded_lens
            ]  # make a new copy where -1 is prior and 0 is first output
            ...
            for t in range(-1, seq_len, 1):  # -1 is prior
                if t == -1:
                    agg_out, hidden_state = (
                        torch.cat((prior_mean, prior_logvar), dim=-1),
                        hidden_state,
                    )
                else:
                    to_agg = h[
            ...
            assert agg_outputs.shape[:2] == (1, batch_size), (
                agg_outputs.shape,
                (1, batch_size),
            )
            assert hidden_states.shape[:2] == (1, batch_size), (
                agg_outputs.shape,
                (1, batch_size),
            )
