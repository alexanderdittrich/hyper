...
blue, orange, green, red, purple, brown, pink, grey, yellow = sns.color_palette(
    n_colors=9
)
black = (0, 0, 0)
COLOR_PALETTE = defaultdict(
    lambda: black,
    {
        "RNN": grey,
        "RNN+S": red,
...
HYPER_LIST = [
    ...
    "HyperNet_no_init",
    "HyperNet_init_b",
    "RL2_HyperNet_init_b_avg",
    "FiLM",
]  # "HyperNet_init_wb", "HyperNet_init_w",  "HyperHead", "HyperNet"
...
def simple_name(model_name):
    ...
    model_name = (
        model_name.replace("smallPol_smallEm", "small")
        .replace("nonVar", "N")
        .replace("_FullTaskChance0", "")
        .replace("_TaskChance0", "")
        .replace("_pol_XL", "")
    )
    return model_name
...
def get_name_events_info(args, directory):
    ...
    )  # => Avg_ST_20__03:12_22:56:12
    model_name_and_seed, date = dirname.split(
        "__"
    )  # => Avg_ST_20     and   03:12_22:56:12
...
def get_scalars(t, events):
    ...
    if t not in tags["scalars"]:
        ...
        if replaced_spaces in tags["scalars"]:
            ...
            print(
                "Warning: run without given tag ({}). Available tags:".format(t)
                + str(tags["scalars"])
            )
            return None
    ...
    for model_name, runs in model_name_2_runs.items():
        ...
        if model_name not in MODEL_ORDER:
            print(
                "Warning: model name,",
                model_name,
                "not in MODEL_ORDER list. It will be added at the end.",
            )
        ...
        if model_name not in COLOR_PALETTE:
            ...
            COLOR_PALETTE[model_name] = tuple(0.75 * r.uniform(size=(3,)))
        num_runs = str(len(runs))
        full_name = (
            (model_name + "_" + str(num_runs) + "runs") if args.nr else model_name
        )  # Add number of runs to name in legend
        ...
        max_run_len = 0
        prev_run_info = None
        lr_2_metrics = defaultdict(
            list
        )  # If multiple lr runs, get final rewards for each run, to find best
        for (
            scalars,
            run_info,
        ) in (
            runs
        ):  # runs contains repreats as well as different lr specified in run_info
            ...
            if args.shift_warmups:
                num_frames_warm = (
                    None
                    if num_warmups(full_name) is None
                    else num_warmups(full_name) * args.frames_per_update
                )
                m_scalars = (
                    m_scalars
                    if num_warmups(full_name) is None
                    else [s for s in m_scalars if s.step >= num_frames_warm]
                )
            if args.final_r:
                # metric = scalars[-1].value # (to not smooth first)
                metric = smooth_ys(m_scalars, args)[
                    -1
                ]  # smooth first. # Faster than below and works
        ...
        for scalars, run_info in runs:
            ...
            if run_len < max_run_len:
                print(
                    "Warning, found events missing for a run with {}, dir {}".format(
                        full_name, args.dir
                    )
                )
            ...
            if args.max_events is not None and len(zipped_events) > args.max_events:
                ...
                zipped_events = [
                    zipped_events[int(round(i * scale_down_factor))]
                    for i in range(0, args.max_events)
                ]
            for scalar_event, smoothed_y in zipped_events:
                if args.step is None or (scalar_event.step <= (args.step * 1000)):
                    ...
                    if args.shift_warmups and ("warmups" in full_name.lower()):
                        num_frames_warm = (
                            num_warmups(full_name) * args.frames_per_update
                        )
                    ...
                    if (
                        "lr" in run_info
                    ):  # for now, since numberical lr is not displayed correctly
...
def smooth_ys(scalars, args):
    ...
    if args.smoothing_sz is None:
        ...
        smoothed_y_vals = np.convolve(
            np.pad(y_vals, (len(smooth_window) - 1, 0), mode="edge"),
            smooth_window / len(smooth_window),
            mode="valid",
        )
...
def plot_runs(
    ...
    fig = plt.figure()
    fullname_model_order = [
        model_to_fullname[m] for m in MODEL_ORDER if m in model_to_fullname
    ]
    ...
    plot = sns.lineplot(
        ...
        hue_order=reversed(
            fullname_model_order
        ),  # Reverse so that items higher in legend are plotted on top
    ...
    sorted_model_order = list(
        sorted(
            list(model_2_best_performances.keys()),
            key=lambda model: np.mean(model_2_best_performances[model]),
        )
    )
    plot = sns.barplot(
        x="Model",
    ...
    if args.upper_ylim is not None:
        plot.set_ylim(top=args.upper_ylim)
    out_path = (
        os.path.normpath(args.out) + "_" + metric_str + "_Overall_Performance.png"
    )
...
def main():
    ...
    parser.add_argument(
        "--tag",
        type=str,
        help="the tensorflow tag to plot",
        default="Meta-Episode Return",
    )
    parser.add_argument(
        "--ignore",
        type=str,
        help="ignore directories containing this string",
        default="state_and_conf",
    )
    parser.add_argument(
        "--exclude",
        type=str,
        help="model names to exclude separated by commas with no spaces",
        default="",
    )
    parser.add_argument(
        "--only",
        type=str,
        help="only allow these model names. or allowed preset: HYPER_SET",
        default="",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--max_events",
        type=int,
        help="discard events so we plot maximum of this number of events",
        default=200,
    )
    parser.add_argument(
        "--step",
        type=int,
        help="max number of steps for plot in thousands (k), or None",
        default=None,
    )
    parser.add_argument(
        "--indiv",
        action="store_true",
        help="Whether to plot individual runs instead of confidence intervals",
        default=False,
    )
    parser.add_argument(
        "--best",
        action="store_true",
        help="Whether to display best over lr search (if multiple lr)",
        default=False,
    )
    parser.add_argument(
        "--lr_keys",
        ...
        nargs="+",
        default=["lr_vae"],
        help="The key for the tuned learning rate.",
    )
    parser.add_argument(
        "--nr",
        action="store_true",
        help="Include num runs in model legend",
        default=False,
    )
    parser.add_argument(
        "--lr",
        action="store_true",
        help="Show lr in legend if grid search done",
        default=False,
    )
    parser.add_argument(
        "--final_r",
        action="store_true",
        help="Use final reward instead of Average Return as metric for best lr",
        default=False,
    )
    parser.add_argument(
        "--AUC",
        action="store_true",
        help="Use AUC instead of Average Return as metric for best lr",
        default=False,
    )
    parser.add_argument(
        "--no_show",
        action="store_true",
        help="Do not show plot on webserver, just save",
        default=False,
    )
    parser.add_argument(
        "--no_legend", action="store_true", help="Do not show legend.", default=False
    )
    parser.add_argument(
        "--no_ylabel", action="store_true", help="Do not show ylabel.", default=False
    )
    parser.add_argument(
        "--log_scale", action="store_true", help="Plot in log scale", default=False
    )
    parser.add_argument(
        "--full_names",
        action="store_true",
        help="Do not use simple_name() function",
        default=False,
    )
    parser.add_argument(
        "--out",
        type=str,
        help="file path for output. Default only works on my old VM.",
        default=None,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--upper_ylim",
        type=float,
        help="Upper ylim for plot. E.g. 10e-30.",
        default=None,
    )
    parser.add_argument(
        "--smoothing_sz",
        type=float,
        help="Size of smoothing window as fraction of data. 1 is all data.",
        default=0.1,
    )
    parser.add_argument(
        "--frames_per_update",
        type=int,
        help="number of frames in one update. Used for shifting plots with pre-compute",
        default=960,
    )
    parser.add_argument(
        "--shift_warmups",
        action="store_true",
        help="Shift plots with pre-compute specified in name of model.",
        default=False,
    )
    parser.add_argument(
        "--metric_tag",
        type=str,
        help="Pick best plot by this metric instead of --tag.",
        default=None,
    )
    ...
    (
        ...
        y_label,
        legend_name,
    ) = get_clean_data(model_name_2_runs, args)
