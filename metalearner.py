...
class MetaLearner:
    ...
    def __init__(self, args, make_logger=True):
        ...
        )  # NOTE: THIS IS THE RL AGLO, e.g. PPO, not policy.py
        task_decoder_dim = None
        if self.args.ti_coeff is not None:
            task_decoder_dim = (
                self.policy.actor_critic.task_in_dim
                if self.args.ti_target == "task"
                else self.policy.actor_critic.get_bn_param_dim()
            )
        self.vae = VaribadVAE(
            self.args,
            self.logger,
            lambda: self.iter_idx,
            task_decoder_dim=task_decoder_dim,
        )
        self.policy_storage = self.initialise_policy_storage(
            hidden_size=self.vae.encoder.agg.state_size
        )
        self.policy.set_vae_optimizer(
            self.vae.optimiser_vae
        )  # do this after initializing policy, instead of passing to init, to get self.policy.bn_param_dim
    ...
    def initialise_policy(self):
        ...
        ti_reset = (
            ...
            and self.args.ti_hard_reset
            and (self.args.task_num_warmup is not None)
        )
        if hyper_reset or ti_reset:
            reset_parameters_ac = dict(
                Policy(
                    args=self.args,
                    #
                    pass_state_to_policy=self.args.pass_state_to_policy,
    ...
    def train(self):
        ...
        for self.iter_idx in range(self.num_updates):
            ...
            assert (
                not self.args.pass_latent_to_policy
                or len(self.policy_storage.latent_mean) == 0
            ...
            for step in range(self.args.policy_num_steps):
                ...
                [next_state, belief, task], (rew_raw, rew_normalised), done, infos = (
                ...
                if len(done_indices) > 0:
                    next_state, belief, task = utl.reset_env(
                        self.envs, self.args, indices=done_indices, state=next_state
                    )
            ...
            if self.args.precollect_len <= self.frames:
                ...
                if (
                    self.args.pretrain_len > 0
                    and not vae_is_pretrained
                    and not self.args.disable_decoder
                ):
    ...
    def encode_running_trajectory(self):
        ...
        return (
            latent_sample.squeeze(0),
            latent_mean.squeeze(0),
            latent_logvar.squeeze(0),
    ...
    def get_value(
        ...
        return self.policy.actor_critic.get_value(
            ...
            latent=latent,
            training=training,
            update_num=self.policy.update_index,
        ).detach()
    ...
    def log(self, run_stats, train_stats, start_time):
        ...
        if self.iter_idx % self.args.eval_interval == 0:
            ...
            for mode in modes:
                ...
                returns_per_episode, extra_dict = utl_eval.evaluate(
                    args=self.args,
                    policy=self.policy,
                    ret_rms=ret_rms,
                    iter_idx=self.iter_idx,
                ...
                # Use unbiased=False when we have only 1 sample to avoid warning
                returns_std = (
                    returns_per_episode.std(dim=0, unbiased=False)
                    if returns_per_episode.shape[0] > 1
                    else returns_per_episode.std(dim=0, unbiased=False)
                )
                mode_tag = "" if mode == "train" else "_test"
                for k in range(len(returns_avg)):
                    self.logger.add(
                        "return_avg_per_iter" + mode_tag + "/episode_{}".format(k + 1),
                        returns_avg[k],
                        self.iter_idx,
                    )
                    self.logger.add(
                        "return_avg_per_frame" + mode_tag + "/episode_{}".format(k + 1),
                        returns_avg[k],
                        self.frames,
                    )
                    self.logger.add(
                        "return_std_per_iter" + mode_tag + "/episode_{}".format(k + 1),
                        returns_std[k],
                        self.iter_idx,
                    )
                    self.logger.add(
                        "return_std_per_frame" + mode_tag + "/episode_{}".format(k + 1),
                        returns_std[k],
                        self.frames,
                    )
                self.logger.add(
                    "Meta-Episode Return" + mode_tag.replace("_", " "),
                    returns_avg.sum(),
                    self.frames,
                )
                self.logger.add(
                    "Meta-Episode Success Avg" + mode_tag.replace("_", " "),
                    success_ret_avg.mean(),
                    self.frames,
                )
                self.logger.add(
                    "Meta-Episode Success Frac" + mode_tag.replace("_", " "),
                    success_in_ep.mean(),
                    self.frames,
                )
                # grad norms
                if grad_dict:
                    self.logger.add(
                        "Grad_Norm" + mode_tag,
                        torch.tensor(grad_dict["all"]).mean(),
                        self.frames,
                    )
                    if self.args.pass_state_to_policy:
                        self.logger.add(
                            "State_Grad_Norm" + mode_tag,
                            torch.tensor(grad_dict["state"]).mean(),
                            self.frames,
                        )
                    if self.args.pass_task_to_policy:
                        self.logger.add(
                            "Task_Grad_Norm" + mode_tag,
                            torch.tensor(grad_dict["task"]).mean(),
                            self.frames,
                        )
                        if self.args.pass_state_to_policy:
                            self.logger.add(
                                "State_Div_Task_Grad_Norm" + mode_tag,
                                torch.tensor(grad_dict["state"]).mean()
                                / torch.tensor(grad_dict["task"]).mean(),
                                self.frames,
                            )
                    if self.args.pass_latent_to_policy:
                        self.logger.add(
                            "Latent_Grad_Norm" + mode_tag,
                            torch.tensor(grad_dict["latent"]).mean(),
                            self.frames,
                        )
                        if self.args.pass_state_to_policy:
                            self.logger.add(
                                "State_Div_Latent_Grad_Norm" + mode_tag,
                                torch.tensor(grad_dict["state"]).mean()
                                / torch.tensor(grad_dict["latent"]).mean(),
                                self.frames,
                            )
            ...
            print(
                f"Updates {self.iter_idx}, "
                f"Meta-Episode Return {meta_r_train: .2f}"
                + (f" (test: {meta_r_test: .2f})" if "test" in modes else "")
                + ", "
        ...
        if (self.iter_idx % self.args.log_interval == 0) and (train_stats is not None):
            ...
            self.logger.add(
                "state_grad_norm_div_task",
                train_stats[4] / max(train_stats[5], 1e-5),
                self.iter_idx,
            )
            self.logger.add(
                "state_grad_norm_div_latent",
                train_stats[4] / max(train_stats[6], 1e-5),
                self.iter_idx,
            )
            ...
            for [model, name] in [
                ...
                if model is not None:
                    ...
                    self.logger.add(
                        "weights/{}".format(name), param_mean, self.iter_idx
                    )
                    if name == "policy":
                        self.logger.add(
                            "weights/policy_std",
                    ...
                    if all(p.grad is not None for p in param_list):
                        ...
                        self.logger.add(
                            "gradients/{}".format(name), param_grad_mean, self.iter_idx
                        )
